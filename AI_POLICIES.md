...
## Current synthesis — 2026-08-13 20:40 EDT
...
- **Other live strength tests:** pair-aware continuation runs on Air and S4
  tranche two runs on Cloud. T4 is terminal `SELECT_NONE`. The old S6 Air
  preflight remains queued behind Pair, while a separate opened-DEV V2 packet
  on the idle performance host awaits its own packet attestation.
...
| **PAIR-BALLOT** | **Retain legal pairs on the ballot** | Keep a legal pair from being crowded out before search can price it. | **Real, rare, defender-heavy gap; successor designs under review** | Census found 15,187 omissions across 18.6M SmartBot-trajectory leads (`0.0816%`), 97.6% early. The 1,024 selected rows are 1,023 defender and one attacker; the defender rows span 990 deal clusters. | Merged PR #86 pins the scored-design boundary. Draft PR #100 specifies a controller boundary; draft PR #101 freezes an exact-champion natural split/role/band dose census. Both are design-only: no implementation, packet, evidence access, scoring, REPORT, aggregation, retry, strength, training, promotion or deployment is open. |
...
human action. These are proposal diagnostics only, pending review; they grant
