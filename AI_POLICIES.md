...
| BANKER_KITTY | **ON** (correctness) | the banker counts its OWN buried cards as known | **NO VALID MEASUREMENT YET** — the three duels (50%/46%/54%) were run while this flag silently disabled banker MC search entirely (0 worlds sampled; incident 2026-08-03-banker-search-disabled), so they measured no-search vs search, not the kitty question | kept because it is strictly true information the sampler already used; the duel must be re-run on the fixed code before any strength claim |
