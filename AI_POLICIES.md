...
| 08-11 | Teacher Stage-C expanded training | Train matched eight-seed cohorts on 7,040 states, comparing the original all-pairs ranker with a direct “beat candidate zero” loss, and select whole cohorts without opening REPORT. | **TERMINAL EXTERNAL PASS / DESIGN-CALIB CAPABILITY** | 96/96 cells and 576 checkpoints independently replayed; selected epoch-32 `all_pairs_v1` bury ranker, 8/8 positive seeds, median improvement `+0.016418` versus candidate zero; play also had positive DESIGN/CALIB bounds | More data rescued stable offline capability; the new direct loss did not win. The fresh REPORT rows below show that neither bury nor global play ranking transported strongly enough for composition. |
| 08-11 | Teacher expanded REPORT v1 | Price the selected bury ensemble once on the third untouched REPORT tranche using the shared one-shot runtime. | **TERMINAL OPERATIONAL NO-USE / ZERO EVIDENCE** | packet `5ce892db…25f0`; receipt `3c4b1f…74bf`; all eight children returned 2 in argparse because `--expected-git` was omitted; zero labels, predictions, utility or result | This is not a model verdict. The no-retry contract spends the third population. The disjoint repaired v2 below supplied the actual model verdict. |
| 08-11 | Teacher expanded-bury REPORT v2 | Re-run the repaired one-shot protocol on a genuinely disjoint fourth population and ask whether the stable bury cohort improves unseen choices. | **SELECT NONE / INDEPENDENTLY REVIEWED** | 32/32 rows; mean `+0.03381`, one-sided LCB `-0.01525`; zero refusals; 264,128 exact candidate worlds; result `2e21a9bf…72ac4d`, final `126d73cd…e58387` | Training stability did not transport strongly enough on fresh bury choices. Close this cohort and population; no retry, composition or strength claim. |
...
| 08-11 | Powered uncertainty Teacher REPORT | Restrict the best play ranker to all 219 fresh public states where the champion's N=30 choice sat near its production margin, giving the hypothesis enough predeclared power for the CALIB-sized effect. | **SELECT NONE / INDEPENDENTLY REVIEWED** | 219/219 rows; action mean `+0.012129`, SE `0.010109`, LCB `-0.005056`; 274,504/274,504 exact candidate worlds; zero refusals; outcome-NLL improvement `+0.47845`, LCB `+0.44201`; result `e2e774da…b4c5` | The generation learned state/outcome value much better than reliable within-ballot action ordering. No composition or retry. Direct outcome argmax also missed post hoc; next uses must change the target/use—explicit advantage, bounded leaf, or freshly tested horizon specialization. |
...
| 08-10 | Human H0-v3 diagnostic | Fairly price production, human, V11 and matched-random proposals without treating human actions as truth. | **TERMINAL NO-USE / NO AGGREGATE** | 555/557 rows completed; two score-free refusals; aggregate `84ef4400…196c`; no partial utility mining | The experiment produced no usable human/V11 rule. This is an operational no-result, not evidence that human actions are weak; Stage C admits no H0-derived proposer. |
