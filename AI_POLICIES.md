...
| 08-09 | S3c exact-root curriculum | Start exact endgame work at naturally reached one-card endings, then advance to two and three cards only after each smaller problem proves bounded and useful. | **DESIGN PASS / ONE-CARD CONTROLLER IMPLEMENTATION ONLY** | 768 unique-deal score-free roots; one-card legal actions all 1, two-card median/max 2/3, three-card median/max 3/7; 29/29 tests and tamper refusal | Claude passed census/packet exact `23632609…b52a` / `df102428…9eca` at marker commit `084ba7e`. This proves supply, geometry and reviewability—not solver capacity or strength. No solver/screen launch is authorized. |
