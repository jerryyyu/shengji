...
| **Value V2 (2026-09)** | Six honest refusals and no scientific output under the confirmatory protocol (capacity-final, telemetry, slot supply, enum linkage, population allowance falsified at 7,200s). Repairs c3b135ce/2f649070/f7e9044d PASSed. Now in DEV mode (ledger `295136ba`): a resumable end-to-end D64 run retained 255 coverage shards and sealed its first 46 label bundles, then stopped on a select-subfold transition defect. Recovery is staged at `562f87af`; no model exists yet. The frozen 256-slot ledger is coverage-audit evidence only; no missing-slot completion or slot-targeted D256 training follows. |
