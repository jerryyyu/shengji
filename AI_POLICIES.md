...
| 08-10 | Teacher Stage-C design + capture/label implementation | Turn the audit lesson into one finite recipe: keep broad cheap ordinary anchors, spend deeper disjoint root comparisons on hard cases, and conditionally add supported model/mechanism proposals without recursively calling MC inside MC. | **DESIGN + REBIND PASS / CAPTURE-V3 REVIEW OPEN / ZERO STATES** | base `20bdb95` / `f213314a…3b4`; rebind `7018f36` / `b60c4298…7b18`; capture source `0b697b6`, packet `d58a9308…c91`; label source `c98b608`; 1,024 DESIGN / 512 CALIB / 512 REPORT | The curriculum is unchanged and both executable stages exist. Capture still needs its exact v3 external marker; label source still needs readiness review, then the frozen state set, a no-retained-outcome capacity preflight and a packet-specific review. No state, label, model or strength result exists. |
| 08-10 | S4 point-banking full-game screen | In rollouts, when the bot is already winning the trick and can retain a higher winner, let it bank a 5/10/K instead of always spending the cheapest winner. | **SCREEN PASS / CONFIRMATION-PACKET REVIEW AUTHORIZED** | exact-state delta `+5.156` points, LCB `+3.029`; fresh whole-game signed-level utility `+0.086914 +/- 0.056166`, LCB `+0.030748`, `n=2,048`; matched-null-minus-champion exactly `0` | The targeted improvement survived natural complete rounds against live report-LCB and an analysis-identical null. This is the first positive S4 strength screen; it authorizes preparing and reviewing a fresh confirmation packet, not confirmation launch, promotion or deployment. |
| 08-10 | S4 complete-round score-free preflight v2 | Play complete mirrored rounds with the point-banking rollout rule, an analysis-identical null, and the live champion on the same deals and random streams. | **CAPACITY + PACKET PASS / CONSUMED** | 4 clusters in `321.32s`; projected screen `91.40` fleet-hours / `11.42` max-shard hours; treatment/null triggered in both roles with exact dose | Exact `cad3992` and packet `17036e63…1385` correctly sized and bound the now-terminal 2,048-cluster screen. The preflight was capacity evidence, not a separate strength claim. |
...
| 08-10 | Human H0-v3 counterfactual execution | Give human, V11 and matched-random proposals fixed candidate slots and common-world work, then admit only sources that survive the fair comparison. | **REFUSED INCOMPLETE / CLOSED WITHOUT UTILITY** | 555/557 rows completed; two seven-card follow throws refused because the generic analysis enumerator omitted live-ballot actions; exactly 705,750 candidate-world rollouts were accounted; no aggregate utility was published | The one-shot guard exposed a real ballot-membership assumption mismatch. The namespace is closed with no retry and no use of the 555 partial outcomes. H0 therefore admits no human proposal rule to Stage C; the frozen V11, structured and matched-random sources remain. |
...
- S4 complete-round v2 preflight `fcc8b8913d80db5b1fe4bb7d6b727dc722bb7d0f4ec9c8806842535fc43ee060`;
  ...
  aggregate `3c7f27b8466ec9ece73820d21d26349bfd95c4fc17db144b26408db4af6b4268`;
  supervisor final `e188f7e8ee80fe2fc17fee6d79b4eb4c6a41a45713c76825ef707981e30f2b24`;
  terminal progress `97d910c19a01cf392a696446d3dbecbaaadf044c8d97f093ec6c6978ace4c0fb`.
...
- H0-v3 controller v1 packet `13d9a97f9adf26860b9f5e0d4889960c75baa5bf979b939206d2399472ba61fc`;
  ...
  passed externally at `205b6af`. Its single execution terminally refused at
  555/557 rows and published no aggregate utility; the 555 partial outcomes are
  closed and no human-derived proposal source is admitted.
