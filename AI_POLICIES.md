...
| 08-09 | Teacher Stage-C design v3 | Turn the audit lesson into one finite recipe: keep broad cheap ordinary anchors, spend deeper disjoint root comparisons on hard cases, and conditionally add supported human/model/mechanism proposals without recursively calling MC inside MC. | **DESIGN PASS / OPERATIONAL REBIND PENDING** | source `20bdb95`; packet `f213314a…3b4`; 1,024 DESIGN / 512 CALIB / 512 REPORT; 20/33 candidate caps; 10,494,720 maximum candidate-world rollouts; zero states/labels | Claude passed the bounded curriculum design. It still names broken H0-v2 and conditional S3c-v1, so one minimal reviewed successor must bind H0-v3/S3c-v2 before capture-controller implementation. No data or stronger Teacher exists yet. |
...
| 08-09 | Human H0-v3 score-free controller v2 | Keep the same bounded human/model proposal experiment, but make the executable itself refuse weak sampler settings and consume an irreversible admission before it can publish a receipt. | **COMPONENT PASS / OPERATIONAL HOLD** | source `6977dbb`; packet `3f68dc6e…7fcf`; external marker `cc1c293`; 557/557 rows; zero outcomes | Review verified the intended logic, but admission writes an unignored lock that makes the required runtime reopen reject its own dirty tree. No execution is eligible. Replacement H0-v3 source `4ebcd09` / packet `cf074871…35392` is under review. |
...
| 08-09 | S3c exact-root curriculum + one-card controller | Start exact endgame work at naturally reached one-card endings, then advance to two and three cards only after each smaller problem proves bounded and useful. | **DESIGN PASS / CONTROLLER OPERATIONAL HOLD** | 768 unique-deal roots; one-card actions all forced, two-card median/max 2/3, three-card median/max 3/7; v1 packet `f58d23b7…3874` passed component review but cannot reopen after admission | No mechanics run is eligible yet. Replacement S3c-v2 source `4ebcd09` / packet `cafbee43…f23e` repairs the lock seam and is under review; two-card work remains closed. |
...
- Teacher Stage-C-v3 packet `f213314ace8ead497fcaccde150d0694851069b970948a10d0823cf74ceb93b4`;
  source/asset `20bdb95` / `1a29418`; design review passed at `d92f595`, but
  the packet remains operationally blocked on a reviewed H0-v3/S3c-v2 rebind.
...
- S3c census `236326099dc9763c6a5941bcb2a90670c4e23ac390ea07a0e4ec5063fa50b52a`;
  ...
  awaits external review; no solver work is eligible before PASS.
...
- **Human proposal pilot:** exact `9770313` froze score-free H0 v1 at packet
  ...
  `cf074871…35392` fixes that seam and awaits external review.
- **Teacher Stage C:** v1/v2 are superseded design artifacts. Frozen v3 source
  ...
  but its H0-v2 parent and conditional S3c-v1 dependency are operationally
  unusable. A minimal successor must bind reviewed H0-v3/S3c-v2 before
  capture-controller implementation; no capture, label or model work is
  authorized yet.
