...
Last reconciled: 2026-08-12 23:25 EDT. This file is the active execution
...
| **P1 / pair ballot retention** | Keep every legal pair on the lead ballot before discretionary throws or tractors consume the 14-action cap, so a promoted low pair cannot disappear before search evaluates it. | **Artifact/evaluator PASS; capacity design pending external review.** PR #72 at exact head `373de84` uses all 1,023 defender rows across 990 deal clusters, excludes the lone attacker, combines DEV+CALIB, binds exact membership/weights, and labels `0.0816%` as SmartBot-trajectory—not champion—dose. Independent builds on Python 3.11/3.12/3.14 are byte-identical after the `math.fsum` repair. **Left:** obtain external design review; measure champion-natural dose before whole-round inference. No preflight/run authority exists. | Externally reviewed defender-only capacity design | REPORT remains sealed; conditional selector learning is not scored or whole-game strength. Keep this stacked experimental code out of permanent main unless its result proves reusable. |
| **P1 / engine and MC performance — independent enabler** | Make every rollout and round cheaper without changing its move, RNG stream or game result. This lowers production latency and lets fixed fleet-hours test more strength ideas; speed alone is not evidence that a policy is stronger. | **PR #71 repair and stacked compatibility receipt await external review.** Cython `_current_winner` and a `sorted`→`min` micro-edit were slower/noisy and were dropped. Head `093ec33` preserves historical H0/RLCB evidence while the lead prefilter measured roughly 12–16% lower whole-round time. Stacked PR #75 `ee6dc48` binds old/new identities and three complete x86 rounds matched after removing only ballot fingerprints. A separate prepared-world seam measured 3.37% pooled wall gain across nine paired rounds and is under adversarial review. **Left:** review #71 then #75; publish the prepared-world seam only if its broader semantics and source-identity boundaries pass. | Reviewed semantics-preserving performance PRs | Performance-only work, not bot strength. Never benchmark beside sealed strength runs; do not change frozen historical constants or substitute optimized code into sealed experiments. |
| **P1 / documentation and repository hygiene** | Keep one clear source of current truth so compute and review time go to hypotheses rather than reconstructing history. | **Core cleanup merged; current-state refresh pushed.** PRs #43 and #57 merged the main cleanup. Draft PR #64 records the current fleet, incidents, review coordination, and preserves Direct-Q/Suphx as explicit research reserves rather than active reruns. Transient exact-identity monitor PR #68 was closed and its branch removed after the Pair-to-S4 handoff, preventing stale dead code from entering main. **Left:** review/merge PR #64 and archive the resolved review tail only after preserving unresolved requests and every marker still needed by live T4/pair/S4/S6 closeout. | Merged concise backlog/job/daily truth plus a safe handoff compaction point | No evidence marker, live run artifact or unmerged experimental head may be deleted. Experiment-specific monitoring allowlists must not become permanent dead code. |
...
|---:|---|---|---|
| 1 | **S5 defensive point protection** | The old “bots feed double” headline was a normalization artifact, but a narrower defect can remain: after the partner has acted and cannot rescue the trick, the rollout may preserve rank/structure by donating a 5/10/K even though a lower-point legal action exists. | **Portability PASS, execution HOLD after INC-18.** The reviewed census found 58 strict hindsight DEV triggers and PR #70 authorized ten witnesses × 32 seeds. PR #74 proved the exact producer portable to x86, but its request template self-authorized a 41.7-second partial attempt and consumed the one-shot admission; no result exists and `retry_authorized:false`. **Left:** review the distinct-attestation repair, then—only with an explicit new recovery marker—use a new admission/result namespace for one diagnostic. Build a treatment only if the literal champion still donates points. Never reuse the old queue/admission or turn this into blanket “never discard points.” |
| 2 | **MC hand-shape bury search: voids + shuai-pai** | The current heuristic mostly ranks individual cards. The strategic choice is the hand left behind: creating a void enables ruffs, while preserving pairs/tractors and legal shuai-pai can create future control. Points may identify some weak states, but should not define the ballot or objective. | **Source, reusable population and continuation semantics all externally PASSed.** PR #51 preserves hidden-kitty refusal and portable one-world JSON; PR #52 pins a deterministic outcome-blind 32 shape + 32 anchor DEV journal; PR #54 provides actor-visible, non-recursive `all_boss` and boss-or-near continuations with candidate zero and equal baseline work preserved. No census or rollout ran. These are experimental branches; close them after their result unless the journal proves reusable. **Left:** source-only census, then one-state/one-world capacity on a qualified free host. |
