...
|---:|---|---|---|---|
| **DEPLOYED / RESTRICTED TEST** | **Gated W32 serving — Codex, #300 / #310** | Release 20, creator-code opt-in only, unchanged 512MB/shared CPU. Public W32 round completed in 604.808s; 63 bot play turns, compute median/p95/max 5.358/27.763/122.273s, no worker errors. Ordinary lobby responsive. | Retain [rollout evidence](W32_FLY_SERVING.md); investigate long-tail latency before wider access. | MC-LCB remains default; no global strength claim, active-game interruption, resize or extra replica. |
| **ACTIVE** | **Model-to-search follow-up — Codex, coordinated with Claude** | Baseline is selected ACDEF2 v2 `3cd27716`, not a CE-selected replacement. Qualifying its compact package for existing Fly test rooms; ABC remains served until the quiet-window switch. | Investigate (1) prior-assisted candidates/allocation outside PUCT, (2) selective learned continuations and value-scored leaves, (3) actual harvest-runner benefits from serving optimizations, and (4) v3 features grounded in model/search errors. Start with matched saved-state diagnostics; retain v2 and value-only W32 controls. | No global rollout, live-job mutation or automatic large sweep. Prior head is a separate checkpoint, not part of the selected CWV value model. Existing net-rollout/leaf negatives must inform new tests. |
