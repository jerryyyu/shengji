...
|---|---|---|
| **T1 / Lane A production latency — EMPTY-ROOM SHIP HOLD** | Keep the first stronger-than-MC production policy enjoyable to play | Release head `578b2c6` closes failure-safe WebSocket cleanup and the real evaluator-cutoff witness on top of runtime `b315e91`; 100/100 exact native replay and all claim/reconnect/X-ray gates remain valid. Image `latency-578b2c6` is built/pushed at unchanged runtime manifest `dbc97802...c2426a`. Deploy only after a fresh peek proves zero connected humans, then verify live timing and rollback readiness. |
