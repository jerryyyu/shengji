...
Last re-derived: 2026-08-10 19:58 EDT.
...
- **S4:** the point-banking mechanism passed both exact-state testing and its
  ...
  live under receipt `fc6d54e7…1077`. At 19:58 EDT all eight shards were
  healthy at 578/2,048 primary clusters (28.2%). Do not inspect interim
  outcomes or signal/reboot Air; terminal verification and independent result
...
- **Teacher Stage C:** v3–v7 bought exact phase, hand-order, final-trick and
  ...
  result `e2eea8c4…d32d4`; Claude passed the terminal result. Fresh score-free
  label-v2 packet `0d119c84…dc32` passed and the one Mini execution completed
  all 2,048 rows with zero refusals. Aggregate external/internal
  `d0b4397c…6cdb9` / `882baad7…02aac0` preserved correct Teacher fidelity:
  ordinary/hard-tail regret UCBs `0.0000295` / `0.0206932`. Frozen V11 recall
  was only 1/48 (mean `0.02083`, LCB `-0.05799`), so V11 is not a Teacher
  proposer. The original REPORT fold is quarantined because that route choice
  used its audit statistic. Fresh digest-only selection packet
  `7dd0caac…70e69` freezes a disjoint 512-row replacement, and PR #23
  `19ca9aa` is ready to freeze one V11-free DESIGN/CALIB training packet.
  **Blocker:** Claude has not yet posted either raw external PASS marker; the
  similarly named lines in `HANDOFF_REVIEW.md` are Codex request templates.
  No dataset, training packet, receipt, cell or checkpoint exists.
...
- **Capacity:** v1 result `111092b7…cee0` remains a valid terminal sizing
  ...
  capacity. Mini is idle at the two-marker training-packet freeze gate; Air
  runs the separately reviewed S4 fixed replication under receipt
  `fc6d54e7…1077`.
...
| **DONE / T4.2 labels; consumption review-gated** | Price every candidate with a stronger, explicitly bounded Teacher so the learner receives counterfactual ranking signal rather than another imitation label. | **All 2,048 iid-v2 rows completed with zero refusals.** Aggregate `d0b4397c…6cdb9` preserves the useful Teacher labels; fidelity UCBs pass. V11 recalled only 1/48 useful alternatives, so the V11 proposer is rejected and the old REPORT fold is quarantined. **Left:** two genuine raw Claude markers authorizing one V11-free training-packet freeze; do not mistake the request templates for external PASS. | Claude independently posts the fresh-REPORT-selection and label-fidelity-consumption markers; no label rerun | Complete candidate-level DESIGN/CALIB labels with posterior, fold, work and refusal provenance, plus a clean confirmatory replacement population | Label generation is terminal. Consumption opens only after both exact raw PASS markers; no partial utility, V11 proposal inference or old REPORT reuse. |
| **P0 / T4.3 seeded models — packet freeze next** | Learn a stable ranker and calibrated outcome signal from the new Teacher rather than selecting one lucky checkpoint. | PR #23 exact `19ca9aa` is pushed and 424/424 exact Stage-C/Teacher tests pass. It materializes only 1,024 DESIGN + 512 CALIB examples, excludes V11 and both REPORT populations, and schedules 48 play/bury × eight-seed × 25/50/100% cells. **Left:** the two raw parent markers, one immutable packet freeze, separate packet review, then one Mini training execution. No dataset, packet, receipt, cell or checkpoint exists. | After both markers, freeze once in the original label-evidence workspace; request a separate packet PASS; then run at most eight Mini cells concurrently | Eight seeds per surface/head family plus state-count learning curves, seed dispersion and one frozen CALIB selection | No single seed advances. The full-data cohort must meet the predeclared stability/calibration gate before fresh REPORT opens. |
| **P0 / T4.4 fresh untouched REPORT — selection review-gated** | Ask once on unseen states whether the frozen learner improves Teacher choice/regret and calibration before paying for games. | The original REPORT fold is diagnostic-only because it informed the V11-free route. PR #22 packet `7dd0caac…70e69` deterministically selects a disjoint replacement: 512 rows (480 play, 32 bury), zero old state/deal overlap and at least 11 spare rows per quota cell, with no material, labels or utility opened. PR #24 exact `b369aee` provides one-shot execution and verification. **Left:** genuine raw PR #22 PASS, a CALIB-selected model, evaluator packet/review, then exactly one open. | Review selection now; later evaluate only the single DESIGN/CALIB-selected capability | One fresh 512-state REPORT result with ranking regret/coverage and signed-outcome calibration against the live baseline | Exactly one capability passes the frozen fresh-REPORT gate or the generation selects none; no retuning, second look or fallback to the quarantined fold. |
| **P0 / T4.5 composed challenger — source ready behind REPORT** | Let the model focus fresh report-LCB search on one Teacher-supported challenger, while a matched random proposal tells us whether the learned ranking—not merely narrowing the ballot—adds strength. | PR #24 composition-v3 consumes only an independently reviewed fresh REPORT passer, keeps `mc-s0-report-lcb` as fallback, includes a same-work random/null arm and never loads V11. **Left:** terminal fresh REPORT passer, composition packet/review/capacity and one fresh whole-game screen. No packet or run exists. | Execute only from the single fresh-REPORT passer | One mirrored treatment/null/champion result against `mc-s0-report-lcb` | Treatment must have positive one-sided utility LCB versus both champion and null; null must remain compatible with champion. PASS opens confirmation-packet review only. |
