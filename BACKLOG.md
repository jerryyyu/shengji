...
| **P1 / pair ballot retention** | Keep a legal pair from being crowded out before search can price it. | **The score-free capacity result and declarative scored design independently PASSed and are merged.** PR #86 exact head `289fdf0` pins the requested authority/ancestry fixture and opens only a future controller-design review. Champion-natural dose remains required. | Separately reviewed controller design | REPORT remains sealed. No effect estimate, packet implementation/freeze/run, evidence access, scoring, aggregation, retry, strength, training, promotion or deployment authority exists. |
