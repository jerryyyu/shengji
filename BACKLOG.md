...
| **T2.4 / Teacher — AUDIT RUNNING / ADAPTER-V2 REREVIEW OPEN** | Convert a valid terminal audit into a Stage-C contract | Exact Mini launch review passed; one supervisor owns 8/8 audit shards. Claude held `f5fb18f` because copied namespaces remained admissible; exact `5b26c4b` now pins the canonical paths, parent populations, receipt nonce and Python runtime and rejects parent symlinks. Consume only after rereview PASS and independent terminal verification; neither branch launches compute. |
...
4. **ADAPTER-V2 REREVIEW OPEN at `5b26c4b`:** the fresh terminal adapter
