...
Last re-derived: 2026-08-11 01:08 EDT.
...
- **Mini:** the one reviewed 5,504-label expansion is terminal COMPLETE:
  16/16 shards, zero refusals and exact work. Combined with 1,536 retained
  labels, this yields 7,040 training examples. A third untouched 512-state
  REPORT remains sealed; independent result review is the current gate.
- **Air:** the independent S4 point-banking replication remains sealed and
  healthy on eight workers. Last score-free heartbeat: 1,358/2,048 primary
...
- **Repository hygiene:** Xray kitty-bury PR #11 merged. Four superseded
  status-only PRs closed and 13 dead/redundant remote branches removed. The
...
|---|---|---|---|---|
| **P0 / T4-L2 expanded labels** | Generate enough high-quality counterfactual labels to tell whether the first Teacher miss was data-limited, while keeping a new exam untouched. | **Terminal COMPLETE; external review open.** Source `32d94a4`, receipt `48a64759…8efe`; 5,504/5,504 rows, zero refusals, aggregate `3deb3a81…f6ca`. **Left:** independent full replay and raw PASS. | `TEACHER_STAGE_C_EXPANDED_LABEL_RESULT_V1_REVIEW` | Byte-identical aggregate, shard membership and finite-work replay. PASS authorizes one training-packet freeze only. |
| **P0 / T4-M2 matched Teacher objectives** | On the same 7,040 states, test whether more data helps the old loss and whether directly learning “is this move better than candidate zero?” fits the protected production decision better. | **Code-complete in draft PR #29 at `c18b80e`.** `all_pairs_v1` and `candidate0_relative_v2` share states, seeds, initialization, epochs and outcome auxiliary loss; 321 tests and a full read-only label replay pass. **Left:** label-result PASS, then one packet freeze and external controller review. | Reviewed 96-cell packet: 2 recipes × play/bury × 8 seeds × 25/50/100% curves | Packet must be DESIGN/CALIB-only, deterministic, no seed cherry-pick, third REPORT sealed, one-shot admission and no retry. |
| **P0 / T4-M3 train and select** | Learn two whole eight-seed cohorts and select only a stable capability, not a lucky seed or checkpoint. | **Not launched.** Prior generation is terminal SELECT NONE. **Left:** controller PASS, one Mini execution, full checkpoint replay and one cohort/surface/head/epoch selection on CALIB. | Training aggregate plus selected eight-model ensemble or `SELECT_NONE` | At least 6/8 positive action-improvement seeds and positive median; outcome selection also needs calibrated NLL improvement. Smaller curves diagnose scale but cannot select. |
...
| **P0 parallel / S4 replication** | Check whether point banking's first positive whole-game result repeats independently without rerunning an expensive redundant full null arm. | **Live on Air.** Exact `fb6ec1a`, receipt `fc6d54e7…1077`, 8/8 workers; last safe progress 1,358/2,048. **Left:** terminal publication, pinned verify and independent result review. | Independent 2,048-cluster treatment/champion result plus 256 exact-null sentinels | LCB95(treatment−champion)>0; null equals champion on every sentinel; exact dose/work. No retry, extension or automatic deploy. |
| **P1 / S6 shuai-pai sourcing** | Ensure search can price a legal throw in early, middle and late leads instead of silently omitting the tactic. | **Source draft ready.** PR #19 `cfa5a53`; KESP AKQ/876 witnesses plus natural and trump-only late states; 11 focused/58 broader tests passed. **Left:** reproduce live omissions, external semantics review, then bounded equal-work screen. | Public-only deterministic source with ≤8 additions and one legal shuai whenever possible | Literal live ballot/candidate zero first; follow/no-shuai states no-op; a source PASS authorizes only a screen packet. |
...
| **P1 / repo integration cleanup** | Replace the long experimental PR stack with one reviewable integration on current `main`, without deleting run provenance. | PR #11 merged; #10/#12/#21/#25 closed; 13 remote branches pruned. **Left:** finish T4 source/results, build one clean integration branch, run the broad suite, merge, then close ancestors and audit local evidence worktrees before removal. | One current-main integration PR plus branch/worktree inventory | Every retained source path has a consumer; every removed branch is merged/superseded and owns no sole evidence bytes. |
...
- [ ] Consolidate duplicated experiment wrappers only after reference audit.
  Candidate dead paths: `segbatch.py`, `replay_log.pretty_cards`, duplicated
  card/seat helpers and superseded one-shot launch controllers.
