...
Last reconciled: 2026-08-13 02:03 EDT. This file is the active execution
...
| **P1 / pair ballot retention** | Keep a legal pair from being crowded out before search can price it. | **Defender-only design externally PASSed; score-free preflight implementation awaits review.** PR #72 `373de84` uses all 1,023 defender rows/990 deal clusters, combines DEV+CALIB, binds exact membership/weights and labels the `0.0816%` dose as SmartBot-only. PR #79 `6461c66` repairs the first serial/self-authorizing draft: 16 concurrent distinct deals cover every lane and split/band cell; canonical Claude provenance, systemd process ownership, closed telemetry schemas and exact 2× projections are tested. **Left:** external code review, freeze one outcome-free packet, then separate packet review before any preflight execution. Champion-natural dose is still required before whole-round inference. | Reviewed, runnable score-free capacity preflight | REPORT remains sealed; no effect estimate, scored run or strength authority exists. Keep this stacked experimental code out of permanent main unless its result proves reusable. |
| **P1 / engine and MC performance — independent enabler** | Make every rollout and round cheaper without changing its move, RNG stream or game result. This lowers production latency and lets fixed fleet-hours test more strength ideas; speed alone is not evidence that a policy is stronger. | **Two bounded PRs await external review.** Cython `_current_winner`, a `sorted`→`min` edit and the trick-state cache are rejected: the cache's safe repair was 10.56% slower. Incremental Memory is also closed for the current champion—179 constructions consumed only 0.073–0.078% of x86/ARM round time because report-LCB rollouts use HeuristicBot, not Memory-aware SmartBot. PR #71 head `093ec33` measured roughly 12–16% lower whole-round time; corrected receipt PR #75 head `90c5630` binds native identity. Prepared-world PR #77 head `0381081` measured 2.62% lower wall time on six exact-head pairs with identical normalized semantics; the older 3.37% mixed revisions and is retired. **Left:** external review of #75/#77, then re-profile the exact stacked champion before choosing another seam. | Reviewed semantics-preserving performance PRs | Performance-only work, not bot strength. Never benchmark beside sealed strength runs; do not change frozen historical constants or substitute optimized code into sealed experiments. |
...
|---:|---|---|---|
| 1 | **S5 defensive point protection** | The old “bots feed double” headline was a normalization artifact, but a narrower defect can remain: after the partner has acted and cannot rescue the trick, the rollout may preserve rank/structure by donating a 5/10/K even though a lower-point legal action exists. | **Execution remains closed after INC-18.** The reviewed census found 58 strict hindsight DEV triggers and PR #70 authorized ten witnesses × 32 seeds. PR #74 proved the producer portable to x86, but its request template self-authorized a 41.7-second partial attempt and spent the one-shot admission without a result. PR #76 head `e285f47` now makes `run` permanently refuse, pins the canonical spent admission, and separates requests from reviewer attestations. **Left:** external validation-only review of #76. Any actual recovery would still need explicit new retry authority and a new admission/result namespace; never reuse the old queue. |
