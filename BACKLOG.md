...
| **T2 / S3a structured bury — MECHANISM PASS / DUEL CODE REVIEWED+MERGED / PREFLIGHT PACKET REVIEW** | Test whether a much wider, strategy-aware kitty ballot finds better once-per-round decisions | Claude passed repaired core `b5dee2e` and Mini-only controller `0085409`; integration `fcad972` preserves the reviewed hashes and passes 76/76. Literal four-cluster score-free packet `492a1390…4086` freezes canonical Mini, Python 3.14.3, `192/24` screen and `768/96` confirmation caps. Review it now; run it only after Teacher releases Mini. No screen/strength/promotion is authorized. |
...
| **T2.6 / S3a duel — CODE REVIEW PASS + INTEGRATED / PREFLIGHT PACKET REVIEW** | Test whether the state-level structured-bury gain survives full games against production | Exact `b5dee2e` / `0085409` passed; integration `fcad972` passes 76/76. Packet contract `492a1390…4086` freezes four score-free clusters, seed 18,000,000,000, both fleet/shard cap pairs and the canonical Mini command. External PASS may authorize that preflight only after Teacher releases Mini; it cannot launch the screen. |
...
2. **Air:** physically idle, but the current live-parent reopener is bound to
   Mini's historical RLCB-C1 host/Python. Preserve the no-write refusal; Air
   needs a separately reviewed portable-parent attestation before S3 compute.
...
2. **PREFLIGHT PACKET REVIEW:** S3a's reviewed full-game core/controller are
   integrated at `fcad972`; independently review literal contract
   `492a1390…4086`, then wait for Teacher to release Mini before the score-free
   four-cluster preflight;
