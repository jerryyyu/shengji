...
Last re-derived: 2026-08-09 18:05 EDT.
...
- **Human corpus / H0:** reviewed `human_v8` contains 2,830 plays and 45 buries
  ...
  packet `13d9a97f…61fc` sampled zero worlds but is now **HOLD**: the runtime
  did not itself require compiled/strict-void mode and receipt deletion could
  reissue admission. Preserve v1 and freeze a versioned repair; no outcomes
  were computed.
- **Teacher / endgame:** Stage-C v2 is held because it binds obsolete H0;
  rebind/freeze v3 only after a repaired H0 controller passes. S3c's one-card
...
| **P0 / T3.4 H0 controller** | Turn the H0 paper experiment into a reproducible program so every human/model/random proposal is judged on intended shared worlds and partial failures cannot masquerade as results. | **V1 HOLD before outcomes.** All 557 rows replayed, but runtime mode was not self-enforced and deleting the receipt could reopen admission. **Left:** version both boundaries, mutation-test them, freeze a new score-free packet and obtain review PASS. | Repair v1 without touching its immutable packet; do not execute | One replacement packet and external marker covering strict runtime, folds, utility, work/refusal accounting, replay and deletion-proof one-shot authority | PASS freezes the T4.1 execution candidate. T3 stops before execution. |
...
| **T3.4 H0 controller** | Can that counterfactual experiment be run once, exactly as designed, with every candidate source, fold and failure accounted for? | **V1 HOLD:** zero outcomes, but strict compiled/void mode was not self-enforced and receipt deletion could reissue admission. **Left:** freeze and pass v2. | Reproducible machinery for one future counterfactual run | Replacement score-free packet receives external PASS, including strict runtime and deletion-proof one-shot admission. T3 does not execute it. |
...
4. **T3.4 controller v1 held / v2 repair next:** exact producer `931f504`,
   asset `ff277b4` and packet `13d9a97f…61fc` remain immutable with zero
   worlds/outcomes. Add runtime self-enforcement and deletion-proof admission,
   freeze/review v2, then rebind Stage-C-v3. Neither step may execute
