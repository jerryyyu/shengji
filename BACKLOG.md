...
Last reconciled: 2026-08-12 08:02 EDT. This file is the active execution
...
| 2 | **MC hand-shape bury search: voids + shuai-pai** | The current heuristic mostly ranks individual cards. The strategic choice is the hand left behind: creating a void enables ruffs, while preserving pairs/tractors and legal shuai-pai can create future control. Points may identify some weak states, but should not define the ballot or objective. | **Source/evaluator draft ready in PR #51 (`59cc2c6`).** It protects the incumbent, covers every feasible single-suit void, guarantees retained pairs and S6 throws, and scores all combinations on common worlds. Seed 0 yields 28 buries / 415 combos; 42 focused/source tests pass. It separately reports existing-live-lead, widened-lead and expanded-bury references so a heuristic candidate zero is not mistaken for production strength. **Left:** lightweight semantics review, choose a reusable DEV bury population, then run a bounded exploratory screen when a host is free. Partial sampler work is retained diagnostically; no whole-game or deployment claim follows. |
