...
Last reconciled: 2026-08-13 01:26 EDT. This file is the active execution
...
| **P1 / engine and MC performance — independent enabler** | Make every rollout and round cheaper without changing its move, RNG stream or game result. This lowers production latency and lets fixed fleet-hours test more strength ideas; speed alone is not evidence that a policy is stronger. | **Two bounded PRs await external review.** Cython `_current_winner` and a `sorted`→`min` micro-edit were slower/noisy and were dropped. PR #71 head `093ec33` preserves historical H0/RLCB evidence while the lead prefilter measured roughly 12–16% lower whole-round time. Corrected compatibility receipt PR #75 head `90c5630` binds the exact 64-character native-code identity without rewriting history. Prepared-world PR #77 head `0381081` prepares each accepted hidden world once; six fresh exact-head pairs measured 2.62% lower wall time with identical normalized decisions, RNG, counters and transcripts. The earlier 3.37% nine-pair number mixed revisions and is retired. **Left:** external review of #75 and #77, then merge only the independently reproduced pieces. A trick-state cache showed a larger provisional speedup but is HOLD until mutable-state, ordering and serialization correctness are solved. | Reviewed semantics-preserving performance PRs | Performance-only work, not bot strength. Never benchmark beside sealed strength runs; do not change frozen historical constants or substitute optimized code into sealed experiments. |
