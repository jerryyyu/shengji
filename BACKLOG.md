...
| **P1 / pair ballot retention** | Keep a legal pair from being crowded out before search can price it. | **Execution code reviewed; the frozen packet is the active review gate.** Claude PASSed PR #79 `6461c66` at canonical `1cf9521`. Codex copied the complete reviewed score-free population plus all 16 source receipts to the performance host, rebuilt exact design `be21b547…f439`, and froze packet `e054c5e5…e492` / internal `25b1888c…a14b5`. It binds the 16-core x86 runtime, keeps execution false and contains no outcomes. **Left:** packet-specific review requested at canonical `023850d`; only its PASS may authorize one score-free capacity preflight. The parent stack still needs PR #55 exact-head merge readiness. Champion-natural dose remains required before whole-round inference. | Reviewed, runnable score-free capacity preflight | REPORT remains sealed; no admission, result, preflight, effect estimate, scored run or strength authority exists. Packet freeze is not execution authority. |
| **P1 / engine and MC performance — independent enabler** | Make every rollout and round cheaper without changing its move, RNG stream or game result. This lowers production latency and lets fixed fleet-hours test more strength ideas; speed alone is not evidence that a policy is stronger. | **PR #81 and stacked draft PR #83 have promising exact-semantics results; external review remains.** On top of prepared-world PR #77, native cheapest-winner selection reduced six fresh balanced whole-round times by 7.73% (lower bound 6.25%); counting lead pairs once per hand added 4.01% incrementally over three fresh pairs (lower bound 3.29%). The first native-lead head `0bd073c` had a malformed-seat segfault; repaired source `8e698e` passes 31 ARM and 30 x86 boundary/parity tests plus an independent adversarial audit. One ordinary three-pair exact-final batch measured 10.2014% lower wall time and 11.3603% higher throughput with exact normalized gameplay/search/RNG behavior. Draft PR #83 head `69ff44e` is CI-green and stacked on #81. Its remote raw evidence is owner-writable and has no immutable manifest; the committed receipt hash-pins the observed artifacts, so this remains exploratory performance evidence, not a confirmatory or strength claim. **Left:** external review of #75/#77/#81/#83 and their evidence boundaries. Nothing is deployed or inserted into a sealed run. | Reviewed semantics-preserving performance PRs | Performance-only work, not bot strength. Never benchmark beside sealed strength runs; do not change frozen historical constants or substitute optimized code into sealed experiments. |
