...
| **CLOSED** | **BELIEF R4 / R5** | Original R4 test scored +21.40% Brier but failed the label control; outer resource/integrity refusal. Separate #179 DEV diagnostic: `NO_PRIMARY_POLICY_SIGNAL` (weights ≈ uniform, 1/104 flips, paired value exactly 0). | Finish [reviewed close-only retirement #217](https://github.com/jerryyyu/shengji/issues/217#issuecomment-5587402153); [preservation scope](RL_PLAN.md#r4-and-r5) is verified, not a full Mini dataset copy. Any research reopening needs separate justification. | No research restart or artifact deletion. |
