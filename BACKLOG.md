...
| **T2 / S3a structured bury — MECHANISM PASS / DUEL CORE+CONTROLLER REVIEW** | Test whether a much wider, strategy-aware kitty ballot finds better once-per-round decisions | The 512-state mechanism passed all three frozen LCBs. Fresh full-game core `3e5fcc0` and corrected Mini-only score-free controller `7b52d19` are pushed with 59/59 combined tests and two bounded reviews open. Air cannot authenticate the historical Mini-bound live parent; do not weaken it or launch there. No preflight/duel/promotion is authorized yet. |
...
| **T2.6 / S3a duel — CORE + MINI PREFLIGHT CONTROLLER REVIEW OPEN** | Test whether the state-level structured-bury gain survives full games against production | Exact `3e5fcc0` freezes 2,048/8,192 fresh mirrored deal clusters, exact report-LCB parent/opponent, structured treatment, champion and matched-null controls, clustered utility gate and score-free sizing. Exact `7b52d19` wraps sizing in a Mini-only one-shot receipt/final. Resolve both reviews, then freeze literal budgets/command; no compute is authorized yet. |
...
2. **REVIEW OPEN:** S3a's 512-state mechanism PASS now has full-game core
   `3e5fcc0` plus Mini-only score-free controller `7b52d19`; resolve both
   reviews and freeze literal budgets before any preflight;
