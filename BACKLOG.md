...
| **P0 / S4 fresh point-banking confirmation** | Make simulated players collect points with a winning point card when it is safe, so Monte Carlo prices realistic point flow. Accumulate evidence prospectively instead of discarding several small positive runs. | **Mini successor ready; review open.** The reviewed design still uses automatic looks at 8,192 and 16,384 fresh clusters. Air's pair screen may outlive Mini's T4 run, so stacked PR #53 `3e668fb` gives the same design a separately named Mini controller, schemas, review marker and preflight namespace. Thirty-six focused tests pass; exact source-only Mini runtime authentication reopened the live report-LCB parent with Python 3.14.3 and the pinned native binary. No preflight or gameplay ran. At true `+0.03`, maximum power is about 84.7%. **Left:** Mini controller PASS; after T4 seals, one score-free Mini preflight; packet review; then the authorized sequence. | Reviewed, Mini-runnable two-look packet with measured capacity; ultimately a terminal fresh sequential verdict | No historical pooling. Look 1 stops PASS if LCB > 0, otherwise automatically continues when clean; final look PASS/SELECT_NONE/HOLD is mechanical. The old Air controller review cannot authorize the Mini profile. |
...
|---|---|
| **S4** | A reviewed score-free Mini preflight and frozen sequential packet ready to launch, or a concrete external HOLD with a repaired packet pushed for review. |
...
| 2 | **MC hand-shape bury search: voids + shuai-pai** | The current heuristic mostly ranks individual cards. The strategic choice is the hand left behind: creating a void enables ruffs, while preserving pairs/tractors and legal shuai-pai can create future control. Points may identify some weak states, but should not define the ballot or objective. | **Base composition passed; current delta review pending; resumable DEV driver ready.** Claude checked PR #51 at `59cc2c6`; current head `a1d107b` adds only hidden-kitty refusal and portable one-world JSON, but still needs an exact-head delta PASS. Stacked PR #52 at `fd7b434` pins the opened 512-state S3a banker asset, proposes 32 diverse shape rows + 32 anchors, validates work, and journals each completed state immutably. A one-state capacity pass can extend later without discarding work; changed code/runtime/work refuses reuse. Twenty-eight focused tests pass and no real census or rollout ran. **Left:** narrow #51 delta review, bounded #52 review, source-only census, then a one-state/one-world capacity measurement after a host frees. Later rollout shuai-pai remains a separate hypothesis. |
...
- Mini owns T4 until its supervisor-final seal. Short work under one hour
  prefers Mini only when it does not contend with the live T4 run. The reviewed
  S4 Mini preflight is first in line after T4 because Air's pair screen is
  projected to run longer.
