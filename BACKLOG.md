...
|---|---|---|---|---|
| **P0 / T4 mid/late Teacher hybrid** | Use the learned model as a proposal inside Monte Carlo search after trick five, so the model contributes ideas while fresh simulations and the live move protect against bad guesses. | **Running on Mini; safe counter-only lower bound 6,396/12,288 sequential arm-rounds (52.05%) complete.** Treatment is complete; seven matched-null shards report 300/512 and one reports 200/512. A 256-state test beat both live and same-work random widening. The slow null tail projects near 16:59 EDT; the champion must then average no more than 26.7 seconds per cluster before the 20:46 child deadline, so the run remains knife-edge. Mini is monitor-only. Claude exact-head PASSed read-only closeout helper PR #80 `e61975c`; 22/22 tests pass both plain and under `SHENGJI_FAST=1`, and the helper is merged on main with no execution authority. **Left:** finish matched null and champion; review the score-free supervisor final; admit one aggregate; obtain the independent terminal verdict. | Reviewed whole-game verdict versus literal live champion and matched uninformed proposal | Treatment must have positive one-sided 95% lower bounds versus both controls, exact work, both-role coverage and no integrity failure. A screen PASS opens confirmation design only. |
| **P0 / S4 fresh point-banking confirmation** | Make simulated players collect points with a winning point card when it is safe, so Monte Carlo prices realistic point flow. Accumulate evidence prospectively instead of discarding several small positive runs. | **Running on Cloud; reviewed score-free progress is 6,187/8,192 clusters (75.52%) at look one.** All 16 workers are healthy. Packet `65c3cf8a…916e8` and the complete 300-billion interval remain quarantined after INC-15; the disjoint 360-billion run owns Cloud with sealed outcomes. **Left:** let the reviewed controller stop or continue automatically at look one, then obtain score-free and terminal external reviews. | Terminal fresh sequential verdict from the reviewed 16-shard 360-billion confirmation | The packet marker authorizes one automatic run only. No old namespace/seed may be retried, no reviewer may call a gameplay-capable path, and any controller/integrity mismatch exits HOLD. |
...
| **P0 / pair-aware rollout** | Remember which higher pairs have been exhausted inside simulated continuations, so search recognizes when a low pair has become unbeatable rather than treating every rollout as memoryless. | **Powered whole-game screen running on Air; reviewed score-free progress is 2,064/7,168 clusters (28.79%).** The reviewed packet fixes 8×896 and ~84% planning power at `+0.05`; all eight workers are alive, outcomes remain sealed and the current pace remains on a substantive timeout trajectory. Contingency PR #85 exact head `111314f` PASSed external design-only review with 40 tests; it grants no capacity screen, replacement launch, retry, extension, scoring or strength authority. **Left:** monitor without interference; preserve either natural completion or the concrete terminal timeout/HOLD. | One complete reviewed treatment/null/champion whole-game screen | Actor-available information only; no hidden-hand leakage. Matched null returns champion at equal work. Level utility is primary; win rate is secondary. Selected-root diagnostics motivate but never establish whole-game strength. |
...
| **P1 / engine and MC performance — independent enabler** | Make every rollout and round cheaper without changing its move, RNG stream or game result. This lowers production latency and lets fixed fleet-hours test more strength ideas; speed alone is not evidence that a policy is stronger. | **PR #77 missed its preregistered retain gate; two PR #81 candidates survive only for rebase and remeasurement.** Exact PR #77 `0381081` reduced six-pair wall time 2.6234% with a 1.5066% one-sided lower bound, below the 3% gate, so it is not retained as-is. On top of that rejected parent, PR #81 `c6c7126` measured 7.7306% lower wall time (lower bound 6.2489%) for native cheapest-winner selection and 4.0124% incremental (lower bound 3.2857%) for one-Counter lead composition, with exact normalized semantics. Split those candidates, rebase them onto the accepted PR #71/#75 path and remeasure the exact stack; do not add the percentages. PR #83 `69ff44e` measured 10.2014% lower wall time over three exploratory pairs, but its raw artifacts are owner-writable and lack an immutable manifest. The performance host is idle. **Left:** review the boundaries, rebase and remeasure retained candidates, and keep #83 exploratory. Nothing deploys or enters a sealed run. | Reviewed semantics-preserving performance PRs | Performance-only work, not bot strength. Never benchmark beside sealed strength runs; do not change frozen historical constants or substitute optimized code into sealed experiments. |
