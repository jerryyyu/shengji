...
| 2 | **MC hand-shape bury search: voids + shuai-pai** | The current heuristic mostly ranks individual cards. The strategic choice is the hand left behind: creating a void enables ruffs, while preserving pairs/tractors and legal shuai-pai can create future control. Points may identify some weak states, but should not define the ballot or objective. | **Source/evaluator semantics passed.** Claude independently checked PR #51's actor boundary, legality, candidate-zero ordering, finite coverage, conservation, common-world accounting and engine-priced failed throws. Current head `a1d107b` adds only a fail-closed impossible-kitty check and portable one-world JSON. Stacked PR #52 (`640f455`) pins the already-opened 512-state S3a banker asset, including its exact source-input/replay manifest, and outcome-blindly proposes 32 diverse shape rows + 32 hash-uniform anchors; it projects work before any launch. Sixteen stacked tests pass; no full census or scoring job ran. **Left:** review #52's population/materialization seam, run the source-only census, then measure a tiny capacity sample after a host frees. Later shuai-pai in continuations remains an explicit separate rollout-realism arm; the current joint candidate covers the immediate post-bury lead. |
