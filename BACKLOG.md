...
Last reconciled: 2026-08-15 07:50 EDT. This file is the active execution
...
|---|---|---|---|---|
| **P0 / BELIEF-V1 B2 calibrated baseline** | Learn an actor-visible distribution over hidden ownership and hand shape, then prove it improves held-out calibration before changing search. | **The post-null milestone is chosen and its scalable infrastructure is built.** B0 contracts, separated actor/target rows, ownership projection and the complete B2 offline controller are in PR #113. Repaired exact head `0da43a0` passes 186 BELIEF tests in pure and strict compiled modes plus the broader engine/parity boundaries; one short external delta review is pending. No corpus, training, test opening or design freeze has occurred. | Exact-head source PASS → byte-preserving merge → fresh Mini V2 design freeze → one marker-reviewed consolidated B2 run and terminal verdict | B2 may establish aggregate hidden-ownership calibration only. No B3 sampler, gameplay, strength, promotion or deployment authority exists. A null closes this recipe; a PASS opens B3 implementation review, not a run. |
