...
Last reconciled: 2026-08-12 10:42 EDT. This file is the active execution
...
| **P0 / S4 fresh point-banking confirmation** | Make simulated players collect points with a winning point card when it is safe, so Monte Carlo prices realistic point flow. Accumulate evidence prospectively instead of discarding several small positive runs. | **Reviewed Cloud preflight running.** PR #56 exact head `6ba6b81` preserves the historical ARM champion identity while binding the Linux/x86 binary through an exact compatibility receipt and full-round golden replays; 82 focused tests pass on both Cloud and ARM. Claude independently passed that exact controller at `368c2bd`. Its one authorized four-cluster score-free preflight was admitted under durable tmux at 10:37; no scored S4 execution is authorized. At true `+0.03`, maximum design power is about 84.7%. **Left:** natural preflight completion; external capacity/result review; freeze and review the sequential packet; then separately authorize the automatic 8,192/16,384-cluster execution. | Reviewed, cloud-runnable two-look packet with measured capacity; ultimately a terminal fresh sequential verdict | No historical pooling. Look 1 stops PASS if LCB > 0, otherwise automatically continues when clean; final look PASS/SELECT_NONE/HOLD is mechanical. Neither the old Air marker nor the Mini marker authorizes Cloud. |
...
- Cloud is the canonical S4 host. Its reviewed score-free preflight currently
  runs alone so capacity timing is not contaminated; the later scored job may
  use 16 workers only after packet and execution review. The completed
  pair-ballot result stays content-closed pending its separate source/schema
  review. Never put the public server address in the repository.
