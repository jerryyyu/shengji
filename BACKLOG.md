...
Last reconciled: 2026-08-12 01:39 EDT. This file is the active execution
...
| **P0 / pair-aware rollout** | Remember which higher pairs have already been exhausted inside simulated continuations, so search recognizes when a low pair has become unbeatable rather than treating every rollout as memoryless. | **Exact diagnostic strongly positive; review open.** PR #42 `d4d8ebd`; 64 fresh exact four-card endgames gave `+9.21875` acting-team points, one-sided LCB `+6.67570`, both roles positive, 30 wins / 1 loss / 33 ties. A live-log census found the public trigger on about 5% of leads, but human and bot observed-choice rates were both about 20%, so recurrence is not preference evidence. **Left:** external source/result PASS, a score-free whole-game dose/capacity preflight, and an adequately powered treatment/null/champion packet. | Reviewed runnable whole-game packet with natural trigger telemetry and a predeclared sample size | Public history plus own hand only; no determinized hidden-hand leakage. Matched null does identical analysis/work and returns champion. Exact diagnostic can open whole-game design, never a strength claim. |
