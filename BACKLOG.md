...
Last re-derived: 2026-08-09 20:15 EDT.
...
- **S4 live strength compute:** the point-banking mechanism passed exact-state
  ...
  `20a420d2…5cc`. At 20:15 all eight workers were CPU-bound with 724/2,048
...
- **Human corpus / H0:** reviewed `human_v8` contains 2,830 plays and 45 buries
  ...
  packet `13d9a97f…61fc` remains **HOLD** with zero worlds. Controller-v2
  components passed at `cc1c293`, but the unignored consumed lock makes the
  required runtime reopen fail. Replacement H0-v3 source `4ebcd09` / packet
  `cf074871…35392` is under review; no diagnostic execution is eligible.
- **Teacher / endgame:** Claude passed Stage-C v3 design source `20bdb95` /
  ...
  10,494,720 nonrecursive candidate-world rollouts. It remains blocked because
  it binds H0-v2 and conditionally names S3c-v1; zero states or labels exist.
  Both share the runtime-lock defect. Replacement S3c-v2 packet
  `cafbee43…f23e` is under review, with zero solver work and no eligible
  mechanics execution. The eventual Stage-C successor should rebind both
  replacements in one allowlisted delta.
...
| **P0 / T3.5 Teacher Stage-C v3 rebind** | Earlier Teachers mostly repeated normal heuristic self-play. Build a mixed curriculum that spends deeper work on uncertainty, novel proposals, bury, point play and tiny endgames. | **Design PASS / bridge code ready / zero states.** Claude passed exact `20bdb95` / `f213314a…3b4`. Source `7018f36` prepares a hash-only bridge binding H0-v3/S3c-v2 without copying or changing curriculum fields; the combined battery is 105/105. **Left:** both controller PASS markers, then real freeze and external bridge review. | Review H0-v3 and S3c-v2, freeze/review the bridge; do not capture or label | One externally passed Stage-C successor bound to executable H0-v3/S3c-v2 while preserving the passed population/label/work contract | Rebind PASS makes capture-controller implementation legal; it still does not create states, labels or training authority. |
| **P0 / T3.6→T4 S3c controller repair** | Four-card exact search exploded past its node cap. Start at forced one-card endings, then grow only after proving capacity. | Claude passed v1's score-free geometry, but its unignored one-shot lock creates the same admit→reopen failure. PR #6 freezes v2 packet `cafbee43…f23e`; 64 roots replay, zero worlds/solver sessions, and the integration test preserves strict Git dirt refusal. **Left:** external PASS/HOLD. | Review PR #6; do not create a mechanics receipt | One executable one-card mechanics controller before any two-card packet | PASS permits one later mechanics/capacity receipt only. Two-/three-card and all strength work remain closed. |
...
| **T3.4 H0 controller** | Can that counterfactual experiment actually proceed after its one-shot admission? | **Reopened: operational HOLD / zero outcomes.** V2's component review passed, but the durable slot makes its own Git tree dirty and runtime refuses. PR #6 v3 repairs and regression-tests the exact admit→reopen seam. **Left:** external PASS/HOLD on `4ebcd09` / `cf074871…35392`. | Reproducible machinery for one future counterfactual run | Satisfied only after H0-v3 external PASS. No T3/T4 receipt before that marker. |
| **T3.5 Stage-C-v3 contract** | Do we have an executable recipe for generating training examples that challenge the live champion instead of repeating ordinary heuristic self-play? | **Design PASS / operational rebind pending / zero states.** Claude passed the exact 2,048-state, finite-work contract at `d92f595`; it still names broken H0-v2/S3c-v1 dependencies. **Left:** bind reviewed replacements without changing the passed estimand. | A reviewed recipe bound to executable human-proposal and conditional endgame controllers | Satisfied only after the minimal Stage-C rebind passes. No state capture, label or training is implied. |
| **T3.6 S3c feasibility asset** | Can sampled exact search work reliably on tiny natural endgames and then grow one card at a time? | **Reopened: operational HOLD / zero solver work.** V1's geometry review passed, but its one-shot lock strands runtime. PR #6 v2 packet `cafbee43…f23e` repairs the boundary. **Left:** external PASS/HOLD. | A bounded executable small-endgame path after S3b's four-card failure | Satisfied only after S3c-v2 external PASS. The mechanics run remains later T4 work and cannot claim strength. |
...
4. **T3.4 reopened operationally:** v2 logic passed but cannot run after its
   one-shot lock dirties Git. PR #6 source `4ebcd09`, H0-v3 packet
   `cf074871…35392` await external review; do not issue a receipt.
5. **T3.5 design passed, execution still blocked:** Claude passed Stage-C-v3
   packet `f213314a…3b4` at `d92f595`, but it binds H0-v2 and conditionally
   names S3c-v1. After both replacements pass, freeze/review one allowlisted
   rebind before capture, labels or training.
...
|---|---|---|---|---|
| **T4.1 H0 outcome** | Discover which human, V11 or random novel moves are genuinely useful under fair search, so Stage C learns from supported alternatives rather than copying style. | Corpus/design are passed, but controller v2 is operationally blocked and no world or utility has been computed. **Left:** pass H0-v3 review, then create its single receipt and execute once, preferably on Mini after S4. | One verified counterfactual bundle over the frozen 384 DESIGN / 128 AUDIT plays and 36/9 buries, with action-source survival and paired human/model/champion utilities | Run only the reviewed H0-v3 packet. Publish all refusals; partial work cannot publish utility. Classify proposal sources rather than treating human agreement as strength. |
| **T4.2 Stage-C dataset** | Create the first clean curriculum whose hard examples and candidate moves can exceed the old heuristic Teacher while preserving routine coverage. | The score-free 2,048-state design passed, but its H0-v2/S3c-v1 dependencies are not executable; zero states/labels exist. **Left:** pass both replacements, review the allowlisted Stage-C rebind, implement/review capture, then freeze and label all rows. | Exactly 2,048 accepted replayable states—1,024 DESIGN, 512 CALIB, 512 untouched REPORT—with 1,920 play and 128 bury rows, rejection counters, action unions and named label provenance | Capture and label only under a separately reviewed Stage-C successor bound to H0-v3/S3c-v2. Hash the dataset/manifest; no training begins if quotas, independence or label budgets drift. |
