...
Last reconciled: 2026-08-12 10:00 EDT. This file is the active execution
...
|---|---|---|---|---|
| **P0 / T4 mid/late Teacher hybrid** | Use the learned model as a proposal inside Monte Carlo search after trick five, so the model contributes ideas while fresh simulations and the live move protect against bad guesses. | **Running on Mini.** A 256-state test beat both live and same-work random widening. The sole 2,048-cluster whole-round screen began 2026-08-11 23:20 EDT; at 10:00 all eight workers remained CPU-bound after 10h39m and every shard's reviewed score-free heartbeat had reached the `treatment 200/512` milestone. Outcomes remain sealed. **Left:** natural completion, score-free supervisor review, one admitted aggregate, and independent terminal result review. | Reviewed whole-game verdict versus literal live champion and matched uninformed proposal | Treatment must have positive one-sided 95% lower bounds versus both controls, exact work, both-role coverage and no integrity failure. A screen PASS opens confirmation design only. |
| **P0 / S4 fresh point-banking confirmation** | Make simulated players collect points with a winning point card when it is safe, so Monte Carlo prices realistic point flow. Accumulate evidence prospectively instead of discarding several small positive runs. | **Cloud successor awaiting review.** Claude passed the Mini controller at `3e668fb`; its authority is authentic but superseded for the current launch order while Mini runs T4. Stacked PR #56 `b0176fc` mechanically rebinds the unchanged automatic 8,192/16,384-cluster design to the 16-core x86 cloud worker. The exact cloud runtime passes 45 focused engine/action/Cython parity and controller tests; no preflight or gameplay ran. At true `+0.03`, maximum power is about 84.7%. **Left:** external cloud-controller PASS plus the requested cross-architecture replay witness; one score-free cloud preflight; packet review; then separately authorized sequential execution. | Reviewed, cloud-runnable two-look packet with measured capacity; ultimately a terminal fresh sequential verdict | No historical pooling. Look 1 stops PASS if LCB > 0, otherwise automatically continues when clean; final look PASS/SELECT_NONE/HOLD is mechanical. Neither the old Air marker nor the Mini marker authorizes Cloud. |
...
| **P0 / pair-aware rollout** | Remember which higher pairs have been exhausted inside simulated continuations, so search recognizes when a low pair has become unbeatable rather than treating every rollout as memoryless. | **Powered whole-game screen running on Air.** Capacity v3 passed after changing 6/8 mirrored roots. The reviewed packet (`4ece02b9…ae47`) fixes 7,168 clusters, 8×896, 500.9 projected fleet-hours and ~84% planning power at `+0.05`. Its sole execution began around 07:24; at 10:00 its detached supervisor and all eight workers remained healthy and CPU-bound after 2h35m; no shard had yet published a terminal record. **Left:** natural completion, score-free supervisor review, one admitted aggregate, and terminal external verdict. | One complete reviewed treatment/null/champion whole-game screen | Actor-available information only; no hidden-hand leakage. Matched null returns champion at equal work. Level utility is primary; win rate is secondary. Selected-root diagnostics motivate but never establish whole-game strength. |
| **P1 / pair ballot retention** | Keep every legal pair on the lead ballot before discretionary throws or tractors consume the 14-action cap, so a rare promoted low pair cannot disappear before search evaluates it. | **Source-only exploration running.** Draft PR #55 `1d6bd2f` adds an off-by-default, ballot-identity-bound retention seam. A local 1,000-round census found omissions only early: 14/4,000 early leads and none across 14,618 mid/late leads. A one-million-round score-free source census is running on Cloud at 16 workers; its unreviewed progress/result contents remain closed. **Left:** independently review the producer/schema, then inspect the terminal source counts and decide whether the rare dose justifies an equal-work screen. | Reviewed prevalence result and explicit advance/stop decision | This repairs ballot availability only. The active pair-aware whole-game lane tests rollout valuation; neither result substitutes for the other. |
| **P1 / documentation and repository hygiene** | Keep one clear source of current truth so compute and review time go to hypotheses rather than reconstructing history. | **PR #43 being finalized.** Full old handoff/backlog history is archived; current handoff names only live lanes and blockers; the fleet probe now covers Mini, Air and Cloud without storing the cloud address. **Left:** merge #43 after CI, then close only terminal/superseded experiment PRs whose evidence and active ancestry are preserved. | Merged docs/monitoring PR plus a verified active-branch/PR inventory | No evidence marker, live run artifact or unmerged experimental head may be deleted. |
...
- Cloud is the canonical next S4 host and may use 16 workers only after the
  exact cloud controller and later packet gates pass. Its current pair-ballot
  census is exploration-only; process identity and file metadata are safe to
  monitor, but its unreviewed progress/result contents stay closed. Never put
  the public server address in the repository.
