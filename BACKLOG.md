...
|---|---|---|---|---|
| **P0 / T4 mid/late Teacher hybrid** | Use the learned model as a proposal inside Monte Carlo search after trick five, so the model contributes ideas while fresh simulations and the live move protect against bad guesses. | **Running on Mini; safe counter-only lower bound 7,296/12,288 sequential arm-rounds (59.38%) complete.** Treatment is complete and all eight matched-null shards report 400/512. A 256-state test beat both live and same-work random widening. The worker cutoff remains about 20:46 EDT, so Mini stays monitor-only and uncontended. Read-only closeout helper PR #80 is merged. **Left:** finish matched null and champion; review the score-free supervisor final; admit one aggregate; obtain the independent terminal verdict. | Reviewed whole-game verdict versus literal live champion and matched uninformed proposal | Treatment must have positive one-sided 95% lower bounds versus both controls, exact work, both-role coverage and no integrity failure. A screen PASS opens confirmation design only. |
| **P0 / S4 fresh point-banking confirmation** | Make simulated players collect points with a winning point card when it is safe, so Monte Carlo prices realistic point flow. Accumulate evidence prospectively instead of discarding several small positive runs. | **Look one completed cleanly and the reviewed controller automatically continued.** Integrity passed; the early-efficacy boundary was not crossed, which is neither a failure nor a final efficacy verdict. All 16 tranche-two workers are healthy and the reviewed score-free lower bound is 448/8,192 (5.47%); outcomes remain sealed. **Left:** finish tranche two, run the pinned read-only verifier and obtain the terminal external review. | Terminal fresh sequential verdict from the reviewed 16-shard 360-billion confirmation | The packet marker authorizes one automatic run only. No old namespace/seed may be retried, no reviewer may call a gameplay-capable path, and any controller/integrity mismatch exits HOLD. |
| **P0 / S6 shuai-pai sourcing** | Keep every legal throw visible, but spend extra Monte Carlo only on a late full-hand boss/near move—the narrow shape that repeatedly showed value. | **Air preflight remains durably queued; scored opened-DEV controller is review-ready.** The queue still waits fail-closed behind broad Pair. Separately, reviewed design PR #91 led to implementation PR #94 exact `3ee600b`: its exact 64 opened-DEV states, disjoint 30-world selection/report folds and 816,480-rollout score-free closeout pass 88 strict compiled tests. Implementation review may authorize packet freeze only; no scored run exists. **Left:** obtain PR #94 review and packet review, while preserving the already-reviewed Air queue for its later four-cluster preflight. | Reviewed Air capacity result and reviewed opened-DEV result; then a powered fresh screen packet | Reused DEV establishes selector feasibility only. Do not create two executable host authorities merely to improve utilization. The fresh whole-game test must use literal live as candidate zero, a behavior-identical matched null, level utility, natural dose telemetry and a separately reviewed population before any strength claim. |
| **P0 / pair-aware rollout** | Remember which higher pairs have been exhausted inside simulated continuations, so search recognizes when a low pair has become unbeatable rather than treating every rollout as memoryless. | **Powered whole-game screen running on Air; every shard is 320/896, totaling 2,560/7,168 clusters (35.71%).** All eight workers are alive, outcomes remain sealed and the current pace remains on a substantive timeout trajectory. Contingency capacity controller PR #93 exact repaired head `0045139` now passes 100 strict compiled tests and awaits delta re-review; PASS may freeze a future packet only and cannot retry or extend this run. **Left:** monitor without interference; preserve either natural completion or the concrete terminal timeout/HOLD. | One complete reviewed treatment/null/champion whole-game screen | Actor-available information only; no hidden-hand leakage. Matched null returns champion at equal work. Level utility is primary; win rate is secondary. Selected-root diagnostics motivate but never establish whole-game strength. |
...
| **P1 / engine and MC performance — independent enabler** | Make every rollout and round cheaper without changing its move, RNG stream or game result. This lowers production latency and lets fixed fleet-hours test more strength ideas; speed alone is not evidence that a policy is stronger. | **Accepted candidates are integrated in PR #89 and await one exact combined-stack measurement.** Exact base is PR #71 `093ec33`; optimized arm `a91eb271` combines native cheapest-winner, guarded native lead, trusted-rollout trick-state maintenance and disabled-exact-endgame hoists. Source/tooling head `fa0f9cf` PASSed. The original host design was retired unrun because `/var/tmp` failed its own parent-ownership admission check. Corrected design SHA `8721aec4…ec9d` moves the absent evidence root beneath root-owned `/var/lib`, validates cleanly and awaits replacement exact-design PASS. All six seeds remain unused. **Left:** run one six-pair N=30/R=300 batch with no retry/tuning; retain only if semantics are exact, aggregate reduction is at least 3% and paired one-sided LCB is positive. | Reviewed semantics-preserving performance PR and immutable benchmark bundle | Performance-only work, not bot strength. Never benchmark beside sealed strength runs; do not change frozen historical constants or substitute optimized code into sealed experiments. |
