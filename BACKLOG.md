...
Last reconciled: 2026-08-13 06:41 EDT. This file is the active execution
...
| **P0 / pair-aware rollout** | Remember which higher pairs have been exhausted inside simulated continuations, so search recognizes when a low pair has become unbeatable rather than treating every rollout as memoryless. | **Powered whole-game screen running on Air; every shard is at the safe detailed floor of 240/896 clusters (26.79%).** The reviewed packet fixes 8×896 and ~84% planning power at `+0.05`; all eight workers are alive and outcomes remain sealed. At 10.98 clusters/hour per shard, natural completion projects around Aug 15 16:58 EDT versus the Aug 14 23:29 timeout. Arms are bundled, so there is no cheaper tail; finishing requires about 41% acceleration and timeout risk is substantive. Draft contingency PR #85 exact head `59119b6` is CI-green with 27 tests; it designs a fresh checkpointed successor only and grants no execution, retry, extension, scoring or strength authority. **Left:** monitor without interference; natural completion if throughput permits, otherwise preserve the concrete terminal HOLD; review—not run—the contingency design. | One complete reviewed treatment/null/champion whole-game screen | Actor-available information only; no hidden-hand leakage. Matched null returns champion at equal work. Level utility is primary; win rate is secondary. Selected-root diagnostics motivate but never establish whole-game strength. |
...
| **P1 / pair ballot retention** | Keep a legal pair from being crowded out before search can price it. | **The score-free capacity result independently PASSed; scored-packet design is the only newly open step.** Claude previously PASSed packet `e054c5e5…e492` at canonical `88866f25`; its sole preflight completed in 157.535 seconds wall / 182.430 seconds CPU with a 543.1 MB peak and left no partial or worker. Claude then ran final read-only PR #84 head `f571146` through the exact result-review CLI and four falsifications before appending raw marker `PAIR_BALLOT_AFFECTED_CAPACITY_PREFLIGHT_RESULT_V1_REVIEW` at canonical `16af447`. **Left:** design—not freeze or run—a scored packet. The parent stack still needs PR #55 exact-head merge readiness and champion-natural dose. | Separately reviewed scored-packet design | REPORT remains sealed. No effect estimate, scored packet freeze/run, scored evaluation, strength, training, promotion or deployment authority exists. The result PASS authorizes design only. |
| **P1 / engine and MC performance — independent enabler** | Make every rollout and round cheaper without changing its move, RNG stream or game result. This lowers production latency and lets fixed fleet-hours test more strength ideas; speed alone is not evidence that a policy is stronger. | **PR #81 and stacked draft PR #83 have promising exact-semantics results; external review remains.** On top of prepared-world PR #77, native cheapest-winner selection reduced six fresh balanced whole-round times by 7.73% (lower bound 6.25%); counting lead pairs once per hand added 4.01% incrementally over three fresh pairs (lower bound 3.29%). The first native-lead head `0bd073c` had a malformed-seat segfault; repaired source `8e698e` passes 31 ARM and 30 x86 boundary/parity tests plus an independent adversarial audit. One ordinary three-pair exact-final batch measured 10.2014% lower wall time and 11.3603% higher throughput with exact normalized gameplay/search/RNG behavior. Draft PR #83 head `69ff44e` is CI-green and stacked on #81. Its remote raw evidence is owner-writable and has no immutable manifest; the committed receipt hash-pins the observed artifacts, so this remains exploratory performance evidence, not a confirmatory or strength claim. A later trick-resolution candidate was retired before timing: the frozen normal harness found 11 of 72 forced no-search plays before the head arm, so no retry or performance claim exists. **Left:** external review of #75/#77/#81/#83 and their evidence boundaries; make future harnesses admit normal forced plays while separately proving the optimized seam is exercised. Nothing is deployed or inserted into a sealed run. | Reviewed semantics-preserving performance PRs | Performance-only work, not bot strength. Never benchmark beside sealed strength runs; do not change frozen historical constants or substitute optimized code into sealed experiments. |
