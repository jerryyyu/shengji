...
| **P0 / T4-M2 matched Teacher objectives** | On the same 7,040 states, test whether more data helps the old loss and whether directly learning “is this move better than candidate zero?” fits the protected production decision better. | **Implementation in draft PR #29.** `all_pairs_v1` and `candidate0_relative_v2` share states, seeds, initialization, epochs and outcome auxiliary loss. **Left:** finish controller/refusal tests, push source, then freeze only after label-result PASS. | Reviewed 96-cell packet: 2 recipes × play/bury × 8 seeds × 25/50/100% curves | Packet must be DESIGN/CALIB-only, deterministic, no seed cherry-pick, third REPORT sealed, one-shot admission and no retry. |
