...
| **P1 / engine and MC performance — independent enabler** | Make every rollout and round cheaper without changing its move, RNG stream or game result. This lowers production latency and lets fixed fleet-hours test more strength ideas; speed alone is not evidence that a policy is stronger. | **V5 was approved and consumed once, then refused before evidence creation or any arm.** The actual root-owned inputs were mode 0644 although reviewed rehearsal copies were immutable; no timing claim exists and V5 never retries. CI-green V6 `cd8eb15` applies the complete root-immutability gate at design freeze, uses fresh seeds/namespace and passes 52 focused plus 89 pure/89 compiled tests. **Left:** exact V6 source PASS, immutable host staging, fresh design PASS, then one six-pair N=30/R=300 batch; retain only on exact semantics, ≥3% aggregate and positive paired LCB. | Reviewed semantics-preserving performance PR and immutable benchmark bundle | Performance-only work, not bot strength. Never benchmark beside sealed strength runs; do not change frozen historical constants or substitute optimized code into sealed experiments. |
