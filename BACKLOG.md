...
Last reconciled: 2026-08-14 10:00 EDT. This file is the active execution
...
|---|---|---|---|---|
| **P0 / pair-aware rollout terminal** | Determine whether actor-visible exhausted-pair memory improves complete rounds, not just selected disagreements. | **The only unresolved strength lane now runs in two disjoint forms.** Air is healthy at 4,640/7,168 (64.73%) but is expected to hit its fixed ~23:29 EDT timeout. Fresh checkpoint packet `f2878fff…a5c9c` PASSed at `95242b4` and started once on Performance Cloud under invocation `ac5425e0…b221`; 16/16 workers are live, first heartbeat 0/224, outcomes unopened. | Reviewed terminal manifest and, only after its own gate, one aggregate/decision | Do not intervene in Air or reuse its partial outcomes. The checkpoint run has no resume or aggregate command; any interruption spends admission. Only sealed terminal review may establish strength. |
| **P0 / post-null roadmap reset** | Choose the next research milestone from evidence rather than keeping idle hardware busy. | **T4, S4 and combined S6 all selected none under clean terminal review. No stronger bot was proved.** The closeout identifies the common transfer failure: selected-state signal, sparse natural dose, weak same-work attribution and continuation fragility. | One reviewed next-goal memo naming a materially different mechanism, its transport argument and its cheapest decisive test | Before another large screen, require natural-dose economics, treatment-vs-champion and treatment-vs-same-work-null evidence, robustness under two continuation/role strata, and detectable whole-game effect. |
| **CLOSED / T4 mid/late Teacher hybrid** | Use the learned model as a proposal inside Monte Carlo search after trick five. | Terminal `SELECT_NONE`; treatment LCB `-0.00759` versus champion and `-0.03313` versus matched null. The uninformed same-work proposal itself beat champion, so learned targeting was not causal. | None for this recipe | No confirmation, retry, promotion or deployment. |
...
| **P1 / pair ballot retention** | Keep a legal pair from being crowded out before search can price it. | **Foundation and two design-only successors are merged.** PR #100 exact `7a27a52` specifies the future scored-controller boundary. PR #101 exact `df93de1` freezes a fresh 8,192-deal, 16-lane exact-champion natural split/role/band dose census; both passed external design review. Neither contains an executable census or scored controller. | Separately reviewed implementation proposals for the controller and natural-dose census | REPORT remains sealed. No effect estimate, implementation, packet freeze/run, evidence access, scoring, aggregation, retry, strength, training, promotion or deployment authority exists. |
| **P1 / engine and MC performance — independent enabler** | Make every rollout and round cheaper without changing its move, RNG stream or game result. This lowers production latency and lets fixed fleet-hours test more strength ideas; speed alone is not evidence that a policy is stronger. | **The exact combined stack cleared its preregistered gate, terminal review and production extraction.** The immutable six-pair N=30/R=300 batch completed once: `111.464s -> 78.782s`, **29.3203% lower wall**, 27.8619% one-sided paired lower bound, all six pairs positive and normalized gameplay/work/RNG/sampler bytes exact. Claude independently reopened all 63 artifacts and VERIFIED/retain at `e5818ee`; PR #98 merged the byte-identical runtime without benchmark machinery. A later current-main diagnostic suggested a similar gain but failed its strict normalizer on an explicit false flag, so it is not a confirmation. | Merged semantics-preserving runtime; use it only in newly reviewed packets and obtain a fresh exact-current-main confirmation before quoting a production-main percentage | Performance-only work, not bot strength. Never benchmark beside sealed strength runs; do not change frozen historical constants or substitute optimized code into sealed experiments. |
...
| **Pair-aware** | Air reaches its frozen terminal/timeout without intervention. The separately reviewed checkpoint successor is now running once on Performance Cloud and must produce a reviewed score-free manifest or an honest spent-admission HOLD. |
...
- Mini's T4 workers and terminal review are complete. Keep the sealed shards
  and sole aggregate closed; no ad hoc reopening, retry or confirmation is
  authorized. Mini is available only for a separately reviewed fresh lane.
