...
Last reconciled: 2026-08-13 20:47 EDT. This file is the active execution
...
|---|---|---|---|---|
| **P0 / T4 mid/late Teacher hybrid** | Use the learned model as a proposal inside Monte Carlo search after trick five, so the model contributes ideas while fresh simulations and the live move protect against bad guesses. | **Terminal independently reviewed `SELECT_NONE`.** All eight shards completed cleanly. Aggregate `f30a77c7…e652` was recursively reproduced and terminally verified at canonical `a165274`. Exact work passed, but treatment-vs-champion LCB was −0.00759 and treatment-vs-matched-null LCB was −0.03313, so neither learned proposal cleared both adoption contrasts. This exact composition is closed. | Closed whole-game verdict versus literal live champion and matched uninformed proposal | No confirmation, retry, promotion or deployment is authorized from this negative whole-game result. |
| **P0 / S4 fresh point-banking confirmation** | Make simulated players collect points with a winning point card when it is safe, so Monte Carlo prices realistic point flow. Accumulate evidence prospectively instead of discarding several small positive runs. | **Look one completed cleanly and the reviewed controller automatically continued.** Integrity passed; the early-efficacy boundary was not crossed, which is neither failure nor final verdict. All 16 tranche-two workers are healthy at 5,318/8,192 (64.92%); outcomes remain sealed. **Left:** finish tranche two, run the pinned read-only verifier and obtain terminal review. | Terminal fresh sequential verdict from the reviewed 16-shard 360-billion confirmation | The packet marker authorizes one automatic run only. No old namespace/seed may be retried, no reviewer may call a gameplay-capable path, and any controller/integrity mismatch exits HOLD. |
| **P0 / S6 shuai-pai sourcing** | Keep every legal throw visible, but spend extra Monte Carlo only on a late full-hand boss/near move—the narrow shape that repeatedly showed value. | **Air preflight remains queued; the opened-DEV V2 packet awaits review.** A loaded-systemd audit retired the unexecuted V1 packet before admission. PR #94 exact `08ee055` PASSed source review at canonical `ec4cdd2`; the controller froze and verified fresh packet `dd7709e9…4adca`, while its packet-review snapshot, admission, records, final and installed unit remain absent. **Left:** packet review, one serial run, sealed records and score-free final review. | Reviewed Air capacity result and reviewed opened-DEV result; then a powered fresh screen packet | Reused DEV establishes selector feasibility only. Do not create two executable host authorities merely to improve utilization. The fresh whole-game test must use literal live as candidate zero, a behavior-identical matched null, level utility, natural dose telemetry and a separately reviewed population before any strength claim. |
| **P0 / pair-aware rollout** | Remember which higher pairs have been exhausted inside simulated continuations, so search recognizes when a low pair has become unbeatable rather than treating every rollout as memoryless. | **Powered whole-game screen runs on Air at 3,520/7,168 (49.11%).** All eight workers are alive and outcomes sealed; timeout risk persists. PR #93's 16-lane capacity attempt correctly refused over cap and cannot retry. Fresh successor PR #96 exact `8a3ef59` preserves all 7,168 clusters and 1.5x safety under an explicit 52-hour cap, binds both the exact fragment and loaded systemd state before admission, and passes 109/109 pure plus 109/109 strict compiled; source review and all run authority remain pending. **Left:** preserve natural Air completion or terminal HOLD; independently review the successor before any fresh packet. | One complete reviewed treatment/null/champion whole-game screen | Actor-available information only; no hidden-hand leakage. Matched null returns champion at equal work. Level utility is primary; win rate is secondary. Selected-root diagnostics motivate but never establish whole-game strength. |
...
| **P1 / pair ballot retention** | Keep a legal pair from being crowded out before search can price it. | **Foundation is merged; two design-only successors await review.** PR #100 exact `7a27a52` specifies the future scored-controller boundary. PR #101 exact `df93de1` freezes a fresh 8,192-deal, 16-lane exact-champion natural split/role/band dose census; its 30 focused tests pass on Python 3.11/3.12/3.14 and the full Pair chain is 205/205. Neither contains an executable census or scored controller. | Independent design reviews, then separate implementation proposals | REPORT remains sealed. No effect estimate, implementation, packet freeze/run, evidence access, scoring, aggregation, retry, strength, training, promotion or deployment authority exists. |
| **P1 / engine and MC performance — independent enabler** | Make every rollout and round cheaper without changing its move, RNG stream or game result. This lowers production latency and lets fixed fleet-hours test more strength ideas; speed alone is not evidence that a policy is stronger. | **The exact combined stack cleared its preregistered gate and terminal review.** The immutable six-pair N=30/R=300 batch completed once: `111.464s -> 78.782s`, **29.3203% lower wall**, 27.8619% one-sided paired lower bound, all six pairs positive and normalized gameplay/work/RNG/sampler bytes exact. Claude independently reopened all 63 artifacts and VERIFIED/retain at `e5818ee`. Draft PR #98 exact `008d75e` extracts the nine byte-identical measured runtime/parity files, removes harness/recovery code, and passes 113/113 pure plus compiled with a non-authorizing receipt. Its CI is green. **Left:** exact merge review; V5 never reruns and V6 is superseded. | Reviewed semantics-preserving performance PR and immutable benchmark bundle | Performance-only work, not bot strength. Never benchmark beside sealed strength runs; do not change frozen historical constants or substitute optimized code into sealed experiments. |
| **P1 / point-flow census and feed anticipation** | Compare human and rollout choices only where each side actually had a legal point-card opportunity, then test narrowly named continuation rules instead of inferring policy from aggregate card counts. | **PR #99 exact `0ee28a0` repairs the PR #95 denominator and provenance boundary.** On 165 private opened rounds, literal legal-opportunity rates are human 164/204 and rollout 14,664/14,666; inferred-strict rates are 32/46 and 302/989. Exact 150-row counterfactual binding found 20 policy flips, only three toward the human action. The former 70%-versus-23% headline is withdrawn. Pure and strict compiled suites are 23/23. **Left:** external proposal/tooling review; no consumer has been selected. | Reviewed descriptive census and a separately designed cheapest honest follow-up | Private opened logs are not sealed strength evidence. The census grants no scored run, training, strength, promotion or deployment authority. |
...
verified the recursively reconstructed `SELECT_NONE`; no T4 continuation is
authorized.
