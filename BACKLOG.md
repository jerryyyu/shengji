...
Last re-derived: 2026-08-07 22:55 EDT.
...
- **Suphx O0 executable implementation is pushed, without freeze or training
  ...
  recommends the capped 64 iterations per arm. Exact `dd83182` now implements
...
| **T1 / Lane C Suphx O0 — IMPLEMENTED / RE-REVIEW REQUIRED / NO RUN** | Test whether a full-information policy can learn before spending compute on feature removal | Preserve preflight SHA `4f0c3dd5…434` and exact O0 material commit `dd83182`; dose is 64 iterations per arm for the exact fixed three-seed ensemble across 128 deal clusters. The repair enforces legal hidden-world witnesses, one exact root and a machine-readable PASS bound to the packet SHA. Routine reopen no longer regenerates 128 DEV games; freeze/review/gate retain semantic replay and all long phases report progress. Focused 22/22, adjacent 193/193 in both routes; broad ordinary 1,026 passed / 27 skipped before the focused O0 deltas. Re-review `dd83182`; on PASS freeze only, then independently hash-admit the packet before Mini training. |
...
2. **Mini:** RLCB-C1 and Direct-Q are terminal. Exact Suphx O0 implementation
   `dd83182` awaits re-review; Mini stays free until code review, packet freeze
