...
| **T2 / S3a structured bury — MECHANISM PASS / TEACHER-GUARDED PREFLIGHT REREVIEW** | Test whether a much wider, strategy-aware kitty ballot finds better once-per-round decisions | Core review remains passed. Self-audit found the first literal packet's Teacher exclusivity was prose-only. Exact `28afa2b` now hashes and enforces terminal-final/no-partial/no-live-worker admission; 15/15 pass and a live probe refuses before namespace creation. Review contract `6185141c…c19c`; no preflight/screen/strength/promotion is authorized yet. |
...
| **T2.6 / S3a duel — CORE PASS / GUARDED PREFLIGHT REREVIEW** | Test whether the state-level structured-bury gain survives full games against production | Exact `b5dee2e` / `0085409` passed and runner bytes remain exact. Superseding `28afa2b` closes the prose-only Teacher-exclusivity gap. Contract `6185141c…c19c` freezes four score-free clusters, seed 18,000,000,000, cap pairs, command and executable Teacher-release guard. External PASS may authorize only that preflight after the guard clears. |
...
2. **GUARDED PREFLIGHT REREVIEW:** independently review exact `28afa2b` /
   contract `6185141c…c19c`. Its live-Teacher refusal is proven and the
