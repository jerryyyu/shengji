...
Last re-derived: 2026-08-11 15:41 EDT from terminal evidence, the weekly
strategy audit and the fresh mid/late screen source.
...
|---|---|---|---|---|
| **P0 / mid/late protected Teacher successor** | Trust the learned model only after five tricks, where the horizon is shorter, and let it offer one idea to report-LCB rather than directly play it. | **Fresh hypothesis; source review open; no evidence authority.** The rule was discovered after REPORT: mid/late was positive on DESIGN (`+0.01189`, LCB `+0.00872`), CALIB (`+0.01248`, LCB `+0.00612`) and the spent REPORT slice (`+0.02255`, LCB `+0.00790`, 97 triggers), while early play was harmful. PR #35/head `3668814` binds trick 5+ before source/scope/model work. Stacked PR #36/head `c9fa22b` adds deterministic natural-state capture, exactly 256 one-state-per-deal triggers balanced across mid/late × attacker/defender, frozen protected/model-null/live decisions, and an independent 3×N=300 final-action evaluation; 123 focused tests pass. **Left:** external source PASS, one separately frozen controller/review, fresh capture/evaluation, then aggregate once. The spent SD (`~0.085`) makes 256 states about 80% powered for a `~0.013` effect. | One 256-trigger fresh state-level composition screen; no large Teacher rebuild | Exact work and zero fallback; treatment-minus-live and treatment-minus-null one-sided LCBs above zero. Null-minus-live explains the added-search effect but is not a third significance gate. The spent post-hoc slice cannot count as confirmation. |
