...
Last re-derived: 2026-08-08 08:50 EDT.
...
| **T1 / Lane B teacher — DIAGNOSTIC REPAIR PUSHED / REVIEW REQUIRED** | Determine whether cheap N=30 choices remain good under the full champion continuation | Audit-v2 remains a consumed operational refusal with no Teacher result. Exact branch `b7534ee` preserves every acceptance rule while splitting safe failure diagnostics and pinning exact-work/reason semantics; 147/147 compiled tests pass. Gate: independent review of material `8ede4d35…43e7`, then a separately reviewed synthetic reproducer and v3 estimand/namespace decision. No fresh audit or evidence-state replay is authorized. |
...
| **T2 / S3a structured bury — REPAIR PUSHED / RE-REVIEW REQUIRED** | Test whether a much wider, strategy-aware kitty ballot finds better once-per-round decisions | Exact `c784e6d` pins the sole remaining accepting field and names the prior exploit in a refusal test; 12/12 focused and 87/87 broad pass. Await exact review of material `34993502…092d`, then—and only after PASS—run the fresh two-state Mini sizing preflight. |
...
boundary. It does not require manufacturing a positive result in one day.
