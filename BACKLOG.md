...
Last reconciled: 2026-08-15 01:24 EDT. This file is the active execution
...
|---|---|---|---|---|
| **P0 / BELIEF-V1 B2 calibrated baseline** | Learn an actor-visible distribution over hidden ownership and hand shape, then prove it improves held-out calibration before changing search. | **The post-null milestone is chosen and its scalable infrastructure is built.** B0 contracts, separated actor/target rows, ownership projection and the complete B2 offline controller are in PR #113. Exact head `529664f4` is green and awaits one consolidated source review. No corpus, training, test opening or design freeze has occurred. | Exact-head source PASS → merge → fresh Mini V2 design freeze → one marker-reviewed consolidated B2 run and terminal verdict | B2 may establish aggregate hidden-ownership calibration only. No B3 sampler, gameplay, strength, promotion or deployment authority exists. A null closes this recipe; a PASS opens B3 implementation review, not a run. |
