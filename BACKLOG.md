...
| **P1 / documentation and repository hygiene** | Keep one clear source of current truth so compute and review time go to hypotheses rather than reconstructing history. | **Current-state refresh complete in this change.** The active handoff and fleet ledger name the terminal T4/S4/S6/Pair outcomes and PR #113's exact review blocker. README, roadmap, backlog, performance, policy, BELIEF, and Suphx status prose now agree while historical evidence remains preserved. | Routine updates at terminal/review transitions; archive only after canonical preservation | No evidence marker, live run artifact or unmerged experimental head may be deleted. Experiment-specific monitoring allowlists must not become permanent dead code. |
...
- Mini's next possible use is the fresh BELIEF-V1 B2 V2 pipeline, but only
  after PR #113 exact-head PASS/merge and a separately reviewed host-specific
