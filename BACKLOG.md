...
| **T2 / S3a structured bury — MECHANISM PASS / DUEL REREVIEW READY** | Test whether a much wider, strategy-aware kitty ballot finds better once-per-round decisions | The 512-state mechanism passed all three frozen LCBs. Claude's consumed-seed HOLD is repaired by fresh core `b5dee2e`; Mini-only controller `0085409` also pins the authenticating Python 3.14.3 runtime. Combined tests pass 61/61 and bounded rereview is open. Air cannot authenticate the historical Mini-bound parent. No preflight/duel/promotion is authorized yet. |
...
| **T2.6 / S3a duel — HOLD REPAIRED / REREVIEW READY** | Test whether the state-level structured-bury gain survives full games against production | Exact `b5dee2e` freezes 2,048/8,192 mirrored deal clusters, explicitly excludes consumed sizing seeds, and starts the fresh screen at 153,000,003. Exact `0085409` wraps sizing in a Python-3.14.3 Mini-only one-shot receipt/final. Resolve the bounded rereview, then freeze both literal capacity caps and the command; no compute is authorized yet. |
...
2. **REREVIEW READY:** S3a's 512-state mechanism PASS now has repaired full-game core
   `b5dee2e` plus Mini-only score-free controller `0085409`; resolve both
