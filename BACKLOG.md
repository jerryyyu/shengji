...
| **P2** | **Docs cleanup** | Done 2026-09-16: README rewritten around release 28 with the pipeline diagram (`docs/visuals/js-m1-one-package.svg`); the two PT-Luna design docs and TEACHER_TOKEN_EFFICIENCY.md live in `docs_archive/` (stubs removed, Jerry's ask). HANDOFF_ACTIVE.md and RESEARCH_PRINCIPLES.md stay until their consumers (AGENTS.md, tooling) migrate. | Consumer migration for the two remaining files. | Nothing else deleted. |
