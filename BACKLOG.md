...
Last reconciled: 2026-08-22 14:59 EDT. This file is the active execution
...
|---|---|---|---|---|
| **P0 / BELIEF-V1 R4 calibrated baseline** | Learn an actor-visible distribution over hidden ownership, then prove it improves held-out calibration before changing search. | **One reviewed scientific DAG is live.** Exact source `d2d466f`, freeze `573fcade…fc16` and admission `21d9cea8…0961` run once on `shengji-cloud`. All 13,312 synthetic rounds and 30 human-capture groups sealed; capture used 82% of its core-hour cap, 85% of storage and 67% of wall, with zero retry/drop. The 12,003-unit input index sealed and tensor-cache construction is underway. | Produce one sealed terminal result, independently reopen it, then publish the full learning-curve/timing diagnosis | No mutation, retry, merge, competing BELIEF run or early outcome opening while live. R4 is offline calibration only; B3/gameplay/strength remain unauthorized. |
| **P1 / BELIEF throughput and reusable artifacts** | Reduce time to learning evidence by using every safe core inside the authorized DAG, then reuse immutable model-independent inputs for later recipes. | R4 measured a 3 h 10 m serial input index. Its serial tensor-cache worker saturates one CPU while disk is nearly idle and projects roughly 10–10.5 h. Capture already uses parallel lanes; training remains the dominant prospective cost. A ranked performance design now distinguishes protocol-preserving parallelism from later distributed/GPU/DAG changes. | After R4 seals, freeze one profile/benchmark matrix; retain parallel index/cache, direct-sparse and cached-I/O changes only when their predeclared thresholds pass, then choose the R5 training topology from the 32-CPU anchor and GPU probe | Never modify R4 or fill its spare cores with unrelated work. Parallel outputs must reconstruct identically in canonical order and retain deadlines, caps, leakage boundaries and corruption witnesses. Use one consolidated source review per retained tranche. |
...
- `shengji-cloud` is reserved by the one reviewed BELIEF R4 offline DAG. Mini,
  Air and Performance Cloud are not part of that packet. Do not launch a
  competing BELIEF or scored job merely for utilization.
