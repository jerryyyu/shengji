...
Last reconciled: 2026-08-13 04:31 EDT. This file is the active execution
...
| **P0 / pair-aware rollout** | Remember which higher pairs have been exhausted inside simulated continuations, so search recognizes when a low pair has become unbeatable rather than treating every rollout as memoryless. | **Powered whole-game screen running on Air; every shard is at 240/896 clusters (26.79%).** The reviewed packet fixes 8×896 and ~84% planning power at `+0.05`; outcomes remain sealed. At 10.98 clusters/hour per shard, natural completion projects around Aug 15 16:58 EDT versus the Aug 14 23:29 timeout. Arms are bundled, so there is no cheaper tail; finishing requires about 41% acceleration and timeout risk is substantive. **Left:** monitor without interference; natural completion if throughput permits, otherwise preserve the concrete terminal HOLD; then follow the reviewed closeout path. | One complete reviewed treatment/null/champion whole-game screen | Actor-available information only; no hidden-hand leakage. Matched null returns champion at equal work. Level utility is primary; win rate is secondary. Selected-root diagnostics motivate but never establish whole-game strength. |
...
| **P1 / pair ballot retention** | Keep a legal pair from being crowded out before search can price it. | **The sole score-free capacity preflight completed; result review is the active gate.** Claude PASSed packet `e054c5e5…e492` at canonical `88866f25`. The preflight completed in 157.535 seconds wall / 182.430 seconds CPU with a 543.1 MB peak and left no partial or worker. Result `544499d1…` / internal `ca36d1af…` is closed to scored use; review was requested at canonical `fe3a59d`. Final self-audit supersession is canonical `5939ef9`; exact PR #84 head `a9a619f17ab80d09083542982f385a2341b0597a` snapshots five dependency names before imports and refuses same-path forged preloads (script `1c65ee1d…`, tests `62e20bb4…`; 89 tests pass). **Left:** independent result PASS, then design—not run—a scored packet. The parent stack still needs PR #55 exact-head merge readiness and champion-natural dose. | Independently reviewed score-free capacity result, then a separate scored-packet design | REPORT remains sealed. No effect estimate, scored packet freeze/run, scored evaluation or strength authority exists. The capacity result may authorize design only. |
...
- Performance Cloud is a separate 16-vCPU x86 host reachable only through the
  local `shengji-perf` alias. It owns isolated profiling and the frozen Pair V3
  capacity packet. That packet pins this host but has no admission or execution
  authority until its packet-specific review PASSes; never copy strength-Cloud
  authority.
