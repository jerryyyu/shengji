...
| **T2.4 / Teacher — AUDIT RUNNING / ADAPTER-V2 REVIEW OPEN** | Convert a valid terminal audit into a Stage-C contract | Exact Mini launch review passed; one supervisor now owns 8/8 audit shards. Review fresh terminal adapter `f5fb18f`, pinned to the literal receipt/preparation and one exact output name, while compute runs. Consume only the first independently verified terminal gate; neither adapter branch launches compute. |
...
4. **ADAPTER-V2 REVIEW OPEN at `f5fb18f`:** the fresh terminal adapter
