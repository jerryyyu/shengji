...
- **Teacher / endgame:** Claude passed Stage-C v3 design source `20bdb95` /
  ...
  it binds H0-v2; zero states or labels exist. S3c-v1 shares the runtime-lock
...
5. **T3.5 design passed, execution still blocked:** Claude passed Stage-C-v3
   packet `f213314a…3b4` at `d92f595`, but it binds H0-v2. After H0-v3 PASS,
...
| **T4.2 Stage-C dataset** | Create the first clean curriculum whose hard examples and candidate moves can exceed the old heuristic Teacher while preserving routine coverage. | The score-free 2,048-state design passed, but its H0-v2 parent is not executable; zero states/labels exist. **Left:** pass H0-v3, review the Stage-C rebind, implement/review capture, then freeze and label all rows. | Exactly 2,048 accepted replayable states—1,024 DESIGN, 512 CALIB, 512 untouched REPORT—with 1,920 play and 128 bury rows, rejection counters, action unions and named label provenance | Capture and label only under a separately reviewed Stage-C successor bound to H0-v3. Hash the dataset/manifest; no training begins if quotas, independence or label budgets drift. |
