...
| **P1 / engine and MC performance — independent enabler** | Make every rollout and round cheaper without changing its move, RNG stream or game result. This lowers production latency and lets fixed fleet-hours test more strength ideas; speed alone is not evidence that a policy is stronger. | **Independent audit PASS; PR #71 published for external review.** Cython `_current_winner` was slower and is dropped. Exact head `414fe29` reduced median ARM round time 16.22% and 20 matched x86 rounds by 11.75%, with byte-identical complete histories/outcomes. It also closes a pre-existing H0 controller source-binding hole for heuristic/actions/bury. Ninety-one strict audit tests and CI pass. **Left:** external semantics/benchmark review, then merge only if independently reproducible. | Reviewed semantics-preserving performance PR | Performance-only work, not bot strength. Never benchmark beside sealed strength runs; do not change the frozen historical H0 constants. |
...
|---:|---|---|---|
| 1 | **S5 defensive point protection** | The old “bots feed double” headline was a normalization artifact, but a narrower defect can remain: after the partner has acted and cannot rescue the trick, the rollout may preserve rank/structure by donating a 5/10/K even though a lower-point legal action exists. | **Diagnostic reviewed and authorized; not yet run.** The reviewed census found 58 strict hindsight DEV triggers; today's candidate-zero and rollout policy both reproduce the same 16, ten in third/fourth seat. Since 57/58 ballots already contain a lower-point action, the likely gap is continuation choice/ranking. Claude PASSed PR #70 `f8083cf` at 21:45, authorizing exactly ten witnesses × 32 deterministic report-LCB seeds on the first noncontended qualified host. **Left:** add the requested direct fixtures before reusing the producer, run the one 320-decision diagnostic, then implement a late-seat treatment/matched null only if the literal champion's final played action still donates points. Never turn this into blanket “never discard points.” |
