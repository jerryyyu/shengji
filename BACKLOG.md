...
Last reconciled: 2026-08-12 06:02 EDT. This file is the active execution
...
| **P0 / S6 shuai-pai sourcing** | Always expose legal throws, but spend expensive search where the added action can plausibly beat the existing ballot. | **A positive selective stratum now exists.** Broad PR #41 remains review-bound; the old 12-override boss/near filters are stopped. On 128 fresh balanced exact endgames, a new boss/near throw that consumed the actor's entire four-card hand beat the best live-ballot action by `+0.234` levels, one-sided LCB `+0.100` (24W/8L/96T; both role means positive). PR #50 preserves the result. **Left:** implement the actor-visible full-hand gate with a belief/search evaluator and run a fresh treatment/null DEV screen; continue the broad capacity review separately. | Runnable full-hand-gated treatment/null DEV packet plus reviewed broad capacity result | Exact perfect-information labels prove action-set value, not a deployable selector. Literal live stays candidate zero; the actor-only evaluator must beat live and matched null in complete rounds before any strength claim. |
