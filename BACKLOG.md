...
Last re-derived: 2026-08-07 22:41 EDT.
...
- **Suphx O0 executable implementation is pushed, without freeze or training
  ...
  recommends the capped 64 iterations per arm. Exact `b5e046c` now implements
...
| **T1 / Lane C Suphx O0 — IMPLEMENTED / RE-REVIEW REQUIRED / NO RUN** | Test whether a full-information policy can learn before spending compute on feature removal | Preserve preflight SHA `4f0c3dd5…434` and exact O0 material commit `b5e046c`; dose is 64 iterations per arm for the exact fixed three-seed ensemble across 128 deal clusters. The 21:57 review caught an impossible-world witness; the repair enforces void/pair/run/declaration/conservation constraints, non-banker burial coverage and a negative illegal-swap test. Follow-ups pin one exact artifact root and require a machine-readable PASS bound to the packet SHA. Focused 22/22, adjacent 193/193 in both routes; broad ordinary 1,026 passed / 27 skipped before the focused admission delta. Re-review `b5e046c`; on PASS freeze only, then independently hash-admit the packet before Mini training. |
...
2. **Mini:** RLCB-C1 and Direct-Q are terminal. Exact Suphx O0 implementation
   `b5e046c` awaits re-review; Mini stays free until code review, packet freeze
