...
Last re-derived: 2026-08-11 04:10 EDT.
...
- **Mini:** the reviewed REPORT v1 commands failed in argparse before evidence
  ...
  spent by the no-retry contract. PR #32 `564db02` fixes the command template
  and selects a fourth untouched 512-row tranche (`3c318da2…41e4`) with zero
  state/deal overlap. Recovery-source review is open; no v2 packet or score
  exists.
- **Air:** S4 replication is terminal positive-but-inconclusive `SELECT_NONE`:
  ...
  External terminal-integrity review remains.
...
| **P0 / T4-R1 untouched REPORT** | Ask once whether the frozen bury ranker transfers to genuinely unseen states before paying for games. | **V1 terminal operational no-use:** eight argparse refusals, zero scores; its third population is spent. **Fresh recovery source ready:** PR #32 `564db02` selects fourth REPORT `3c318da2…41e4` (480/32), zero overlap against three prior populations, and fixes every command template; 334 tests pass. **Left:** recovery-source PASS → freeze/rebuild one score-free packet → separate packet PASS → one look. | One candidate-zero comparison for the exact eight-model bury ensemble on the fourth untouched population | Positive predeclared REPORT LCB and at least one trigger, or `SELECT_NONE`. No retry of v1, tuning, reuse, second capability or pooling. |
...
| **P0 parallel / S4 replication** | Check whether point banking's first positive whole-game result repeats independently without rerunning an expensive redundant full null arm. | **Terminal `SELECT_NONE`, pinned verify clean.** Exact `fb6ec1a`; 2,048 clusters give `+0.048828 +/- 0.055712`, LCB `-0.006884`; 256 null sentinels equal champion. **Left:** external integrity PASS, then close without retry/extension. | Independent terminal result review | Replication did not clear LCB>0. Preserve the positive estimate as diagnostic evidence; no confirmation or deploy. |
