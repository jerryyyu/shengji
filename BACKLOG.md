...
|---|---|---|---|---|
| **P0 / BELIEF-V1 B2 calibrated baseline** | Learn an actor-visible distribution over hidden ownership and hand shape, then prove it improves held-out calibration before changing search. | **The post-null milestone is chosen and its source infrastructure is merged.** B0 contracts, separated actor/target rows, ownership projection and the complete B2 offline controller passed exact review at PR #113 `3ee0eb8` and merged byte-preservingly through main `959c05d`; 187 BELIEF tests pass in pure and strict compiled modes. No corpus, training, test opening or fresh design freeze has occurred. | Fresh Mini V2 design freeze → exact design marker → one consolidated B2 run and terminal verdict | B2 may establish aggregate hidden-ownership calibration only. No B3 sampler, gameplay, strength, promotion or deployment authority exists. A null closes this recipe; a PASS opens B3 implementation review, not a run. |
