...
| **P1** | **Absolute leaf value** | Value V1 cleanly selected none: its labels contained action signal, but 44 fit deals could not support the 212k flat model and held-out action choice was harmful. Draft V2 keeps the canonical MCTS contract `V(any legal state) -> terminal outcome distribution`, uses one state per independent deal, distinguishes state/proposal teachers from the one frozen numeric continuation labeler, and separates label validity, optimizer wiring, data scaling, held-out value learning, and production-action usefulness. | Review `VALUE_AFTERSTATE_V2_ABSOLUTE_LEAF_DESIGN.md`, then run only its score-free exact-32-deal label/teacher-economics census. Implement the learner only if D256 fits; a later full-DAG capacity gate chooses the largest outcome-blind D256/D512/D1024 tier whose exact independent source supply exists and whose canary, nested data curve, natural/control training, audit, and reconstruction project to at most six hours under the immutable 12-hour cap. | No V1 retry or heldout reopening. PT-Sol/Luna/human states require fresh deal clusters and common-policy relabeling. Passing value calibration without beating the production action is recorded separately, not promoted. No PUCT, policy head, BELIEF integration, or one-trick consumer is bundled into V2. |
