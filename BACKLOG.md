...
Last reconciled: 2026-08-12 05:12 EDT. This file is the active execution
...
| **P0 / S6 shuai-pai sourcing** | Always expose legal throws, but spend expensive search where the added action can plausibly beat the existing ballot. | **Broad reviews open; boss/near-only retargeted/stopped.** PR #41 is exact-Air green. PR #47's 32-cluster pilot had 31 ties, one −2 loss and zero wins. Replaying all 12 overrides on the exact report worlds found 276 failed full throws: the loss witness failed on 174/300 worlds, but a zero-failure gate retained only ten neutral all-boss overrides and no wins. **Left:** broad packet review/preflight and source-gate semantics/cost closure. Do not fund a large boss/near-only confirmation; require a materially different evaluator or positive targeted screen. | Reviewed broad capacity result plus a revised, evidence-backed selective-search hypothesis—or a quantified stop for the broad recipe | Literal live ballot stays candidate zero. Treatment must improve over live and matched null; DEV signal, source coverage and cost reduction alone are not strength. |
| **P0 / pair-aware rollout** | Remember which higher pairs have been exhausted inside simulated continuations, so search recognizes when a low pair has become unbeatable rather than treating every rollout as memoryless. | **Capacity measured; review pending.** The exact four-cluster v3 preflight completed once in 503s: v1 changed 6/8 mirrored natural roots and projects a 2,048-cluster screen at 143.1 fleet-hours / 17.9 maximum shard-hours. An independent all-nine changed-root audit at 4,096 fresh common worlds favored v1 in level utility on 5–6 roots, opposed it on 1–2 and left two unresolved; raw v2 adds two good changes but undoes one beneficial v1 choice. **Left:** external capacity-result PASS, then freeze/review a powered v1 packet with v1 protected from raw-v2 reversion. | Reviewed score-free whole-round capacity result and an adequately powered treatment/null/champion design—or a quantified stop decision | Actor-available information only; no determinized hidden-hand leakage. Matched null does identical analysis/work and returns champion. Selected-root diagnostics motivate the lane but never establish whole-game strength. |
...
- Air finished the corrected pair-v3 preflight and the short all-nine diagnostic.
  S4, S6 and the pair screen now wait on distinct exact review markers; their
  namespaces remain untouched and no scored work is authorized.
