...
Last reconciled: 2026-08-13 14:12 EDT from canonical main `e503235`.
...
| **Performance Cloud** | PR #93 score-free concurrent capacity, 16 workers | Claude PASSed PR #89 V3 design `e0a0386c…f4`, but its sole invocation failed before the first arm while staging the base source archive (`KeyError: repo`). V3 is spent with zero gameplay/timing and immutable refusal SHA `f8152b6c…a92`; never restart it. Repaired V4 head `df730d7` is CI-green and awaits source review before any new design freeze. Meanwhile PR #93 packet PASS `e503235` authorized one score-free capacity attempt; it started at 14:08:29 EDT under invocation `19c906f…`, and all 16 workers are saturated. Admission is consumed; capacity/result receipt remain unpublished. Do not overlap another perf job or restart this unit. |
...
1. **PR #89 V4 source repair.** V3's reviewed one-shot failed before its first
   arm while staging the base source archive and is spent. Review exact V4 head
   `df730d7`, which repairs only that pre-arm archival seam and uses six fresh
   seeds. PASS may authorize a fresh V4 host-design freeze only; that design
   still needs its own exact PASS before one six-pair batch.
