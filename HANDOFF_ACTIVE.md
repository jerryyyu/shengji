...
## T3 closeout evidence

- **S3a:** all 2,048 clusters are preserved. The exact verifier recomputed the
  aggregate/final and returned `SELECT_NONE`; its invocation masked only an
  unrelated dirty ledger status bit, so this is accepted for a conservative
  negative close but is not a positive-authority cleanliness precedent.
- **Human corpus:** manifest `b9699790…6553` binds the Fly snapshot and repaired
  public-only encoder. It records 2,830 accepted plays, 45 accepted buries,
  seven rejected incomplete rounds and 25 legal off-ballot actions. Corpus and
  HUMAN-C1 tests pass 66/66.
...
- **Evaluation:** human logs are proposal diagnostics only. DESIGN/AUDIT data
  ...
  people-facing ladder is blinded candidate-versus-champion within the same
  cohort, balanced by role/seat/banker and clustered by player session.

Independent Stage-C rebind marker is at `origin/main` commit `cb9471b`:
...
4. train at least eight seeds of separate ranking and calibrated signed-outcome
   heads, select on DESIGN/CALIB and open REPORT once;
5. compose the one passer inside report-LCB with an incumbent fallback and a
   same-work random/null arm;
6. require fresh whole-game screen and confirmation against
   `mc-s0-report-lcb`, then use the blinded HUMAN-C1 ladder.
