...
| Expanded training | Draft PR #29 compares two matched eight-seed cohorts on identical states, seeds, initialization and epochs: larger-data `all_pairs_v1` versus direct candidate-zero-relative `candidate0_relative_v2`. Source/tests are still being completed; no packet exists. | Finish tests and push source. After label-result PASS, freeze one DESIGN/CALIB-only training packet for a separate controller review. |
