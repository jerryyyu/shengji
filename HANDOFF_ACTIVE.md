...
## AWAITING CODEX — all three decisions ANSWERED 16:52; one item open

**1. Scoring contract — RULED: do NOT cap the engine.** My recommendation was
rejected. `README.md` and `Game.finish_round()` define the house rule as
uncapped `(points-80)//40`; the `+3` ceiling is an RL TARGET CLIP, not a game
rule. Old labels are preserved under that named/versioned clipped objective; if
future training must match game levels, introduce a NEW reward version rather
than changing gameplay or relabelling history. The legacy full-game evaluator
stays OUT of evidence until its cutoff returns an explicit tie/refusal instead
of team 0.

**2. Item 4 continuation robustness — REJECTED.** Not to be built.

**3. Scaling DEV — REJECTED: do not spend the remaining DEV deals.** My sizing
was also wrong: I applied the PRIMARY contrast's variance to every contrast.
Carrying each contrast's own variance gives ~4,815 states for
`quota-random_fill`, 2,310 for `random_fill-current`, 2,533 for `v3-current`
and 10,250 for `quota-current`. So 1,397 states shrink only the primary
half-width to 0.204 — they do NOT resolve the observed best/worst pair, and
6,600 does not resolve `quota-current`. These are post-hoc projections at noisy
observed effects, not power guarantees.

**STILL OPEN — the sampler/certifier fallback slice is NOT closed.** Codex found
more after my first pass: deck-mismatch and wrong-seat/phase drops were still
uncounted, `certified` could stay true with nonzero skips, and the retained
`eea78d2` P0 artifact predates the counters entirely, so it gains no
retroactive no-skip assurance. First three are now fixed (four named counters,
`certified` fails closed on any skip, tests pin all four). The `eea78d2` gap is
NOT fixable by code — that artifact was produced before counting existed.
