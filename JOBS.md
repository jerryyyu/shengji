...
Last reconciled: 2026-08-15 08:26 EDT. This file owns current compute and
...
|---|---|---|
| **Mini** | idle / BELIEF-V1 pre-execution | **NO LIVE RESEARCH JOB.** T4 is terminally reviewed `SELECT_NONE`. PR #113 test-only successor `3ee0eb8` is locally green and awaits exact-head delta review; no BELIEF corpus, training, test opening or design freeze has occurred. A rejected-head V1 design is preserved but must never initialize; the post-merge V2 namespace is absent. |
...
|---:|---|---|
| 1 | BELIEF-V1 B2 offline milestone | **TEST-ONLY DELTA REVIEW PENDING.** PR #113 exact `3ee0eb8`, parent `0da43a0`, adds the two witnesses requested by ledger `01a95d6`; no production byte changed. 187 BELIEF tests pass in both modes. Neutralizing the mid-history chain guard or either terminal mechanics accumulator makes the new exact-value witness fail. PASS permits source merge-readiness and one later Mini-specific V2 design freeze only; a second exact marker then covers the consolidated capture/reference/training/test sequence. |
