...
PAR={256:272716,512:610764,1024:1483468,2048:4015308,
     ...
     244:608252, 340:608294, 436:609296}   # depth-8 residual at the h512 budget (grid S-d8, from its receipt)
...
YT=[0.54,0.56,0.58,0.60,0.62,0.64,0.66,0.68,0.70,0.72,0.74]   # floor lowered 09-13 for M1 (0.59746), 09-14 for G1 (0.54952); the guard refuses anything under CE_LO
CE_HI=0.740   # every CE chart's top; a model above it is listed by name, never drawn off-canvas
CE_LO=0.540   # every CE chart's bottom
for d in R:
...
s,X,Y,x0,x1,y0,y1=frame(880,470,84,200,26,64,XLO1,XHI1,0.540,0.740,YT,XT1,
...
s,X,Y,x0,x1,y0,y1=frame(880,420,84,200,26,70,2.0e5,5.5e6,0.540,0.740,
...
# ---- chart 4b (g8): every policy/value-search comparison vs production, grouped by form.
# Jerry 2026-09-21: chart 4 "does not show the PVsearch vs prod" -> "yeaa add it"; then "fix
# occlusion and include all the PVsearch comparisons vs prod" (the first cut drew a legend on
# top of the interval texts).  Layout: names on the left, bars in the middle, the interval
# text alone on the right, a header row per form; the caption carries the reading guide.
import re as _re
_E8 = []
for _ck, _form, _txt in (globals().get("W64_SEARCH_VS_PRODUCTION") or []):
    _m = _re.match(r'\s*([-+][\d.]+)\s*\[\s*([-+][\d.]+),\s*([-+][\d.]+)\s*\]', str(_txt))
    if not _m:
        raise SystemExit("chart 4b: W64_SEARCH_VS_PRODUCTION[%s, %s] is not 'm [lo, hi]': %r" % (_ck, _form, _txt))
    _dd = next((d for d in R if d["ck"] == _ck), None)
    if _dd is None:
        continue          # a test fixture may swap in rows without this checkpoint (as chart 7)
    _E8.append((_dd, _form, tuple(float(g) for g in _m.groups())))
if not (globals().get("W64_SEARCH_VS_PRODUCTION") or []):
    raise SystemExit("chart 4b: W64_SEARCH_VS_PRODUCTION reached charts.py empty; the builder is not forwarding it.")
_FORMS8 = []
for _, _f, _ in _E8:
    if _f not in _FORMS8: _FORMS8.append(_f)
_LAYOUT8 = []                                  # ("head", form) | ("row", entry), sorted by point within a form
for _f in _FORMS8:
    _LAYOUT8.append(("head", _f))
    _LAYOUT8.extend(("row", e) for e in sorted((e for e in _E8 if e[1] == _f), key=lambda e: -e[2][0]))
_ROWH8 = 34
W8 = 960
L8, R8, T8, B8 = 236, 214, 34, 58
H8 = T8 + _ROWH8 * len(_LAYOUT8) + B8
x0, x1, y0, y1 = L8, W8 - R8, T8, H8 - B8
_v8 = [v for _, _, t in _E8 for v in t] or [0.0]
...
while _t <= XHI8 + 1e-9:
    ...
    s.append('<text x="%.1f" y="%d" class="ax am">%+.2f</text>' % (X8(_t), y1 + 19, _t))
    _t += 0.02
s.append('<text x="%.1f" y="%d" class="reft am">production parity</text>' % (X8(0), y0 - 12))
s.append('<text x="%d" y="%d" class="axl am">signed level utility per round vs production (above zero beats it)</text>' % ((x0 + x1) / 2, y1 + 42))
_n8 = 0
for _k, (_kind, _item) in enumerate(_LAYOUT8):
    _y = y0 + (_k + 0.5) * _ROWH8
    if _kind == "head":
        s.append('<text x="%d" y="%.1f" class="grp">%s</text>' % (x0 - 226, _y + 4, esc(_item)))
        continue
    _d, _f, (_m, _lo, _hi) = _item
    _n8 += 1
    ...
    s.append('<text x="%d" y="%.1f" class="row" text-anchor="end">%s</text>' % (x0 - 14, _y + 4, esc(_d["n"].split(":")[0][:26])))
...
print("W64-search-vs-production chart built:", _n8, "arms in", len(_FORMS8), "forms")
