...
## Codex — 2026-08-07 12:56 EDT — new source gates pass; teacher launch held by stale spec

Reviewed pushed `3e4350a..ced1033`, current `JOBS.md`, and the clean pushed
repair worktree `d1d2019`; no experiment/training job or evidence aggregate was
opened or launched. The frozen-branch V11 repair now has `protocol_problems() ==
[]` and 15/15 tests pass, so its recursion/source gate is PASS; actual artifact
reopening and publication remain NOT RUN/NOT REVIEWED, and the ledger's
“untracked recursion defect” description is stale.

Teacher-v3 mechanics also pass: `be25b4d` repairs the exact 143000001:44:0
witness, preserves the hand, stays in the MC ballot across more than 1,400
two-role lead/follow decisions, and versions the executable packet to
`teacher-v1-entry-149m-v3`. The compiled RLCB/teacher/V11 matrix is 69/69.
Execution is nevertheless **HOLD** because `TEACHER_V1_SPEC.md` still declares
the already-refused 143M-v2 packet and its exact seeds as the fresh authorized
packet. Update and freeze that governing spec/ledger before any v3 capture or
diagnostic launch.

RLCB-C1's committed receipt at `ced1033` passes its exact compiled preflight;
the 60,000,011 null has zero registered stream collisions and the test catches
the historical lag-17 offset. This is a protocol PASS only—`JOBS.md` still says
RUNNING none, and no C1 game was started. Existing CAXI bytes independently
reproduce 138 bot plays/109 searches, p50 1.143s and max 20.499s; the quoted
p95 16.413s is the lower-order-statistic convention (linear p95 16.464s).
That confirms the production-latency P0. No frontend/evaluator fix or new
Cython-parity/duel evidence appeared, so the prior frontend and cutoff-test
HOLDs remain.

---

## Codex — 2026-08-07 15:05 EDT — terminal screens reproduce; Stage-B and latency boundaries need repair

Reviewed all pushed post-12:56 refs through `8588dad` and `1a2a713`, both
ledger views, and only terminal/local artifacts; no live RLCB-C1 or Stage-B
outcome was opened and no game, experiment or training was run. Direct-Q's
sealed SHA `1fa6789e…` independently reopens every saved input and exactly
recomputes SELECT NONE: gameplay is `+0.162760 +/- 0.058755`, but seed 1 and
both pooled role learning LCBs fail. The V11-v2 aggregate bytes are not on this
checkout, so its safe terminal FAIL is ledger-consistent but not independently
reopened here.

Teacher Stage-A's JSON-domain repair and the new progress callback are sound;
the focused transition/progress checks pass 5/5 and progress cannot change the
record bytes or RNG. The N=30 Stage-B run is correctly attribution-only. One
source claim is false, however: `stage_b_source_transition_problems()` pins
ancestry and path names but accepts any new `teacher_v1_states.py` digest; an
arbitrary 64-hex freezer digest returns `[]` when the allowed path set is held.
The actual `d6adbe6` diff changes only the bridge and the frozen `90956d…`
state set was independently recomputed, so this does not burn the running
labels. Before Stage C can be authorized, add an artifact-only check pinning
the exact `d6adbe6` freezer commit and source SHA. The separate report-LCB audit
must also freeze its state rule/weights, policy/ballot/work, independent
selection/report folds and decision threshold before any Stage-B gold outcome
is read; it may not reuse the N=30 gold-report tensor as champion evidence.

The sanitized 100-decision Air replay and its 7/7 tests are valid uncontended
host evidence (p50 0.172s, p95 0.357s, max 0.422s), not Fly evidence. Two
corrections keep the frontend deploy HOLD: the comparator is not literally
eight ULPs—at `0.25`, a 16-ULP change passes because tolerance uses
`ulp(max(1, magnitude))`—and the scheduler file fails reproducibly here (3/6
isolated) because absolute 80–180ms wall-clock assertions include scheduler
jitter. More importantly, no new test proves the 50ms seat-claim grace; it
materially narrows the prior 700ms atomic claim window and the room lock then
blocks claims for the entire search. Require a real WebSocket claim-vs-slow-
search witness plus portable synchronization-based scheduler tests before
deploying. `.dockerignore` safely excludes host `_fast*.so`, but adds no new
native-parity evidence.

Finally, the job ledger is not authoritative yet: main still says RUNNING none,
and the latency-branch `JOBS.md` says Stage B is not launchable while
`HANDOFF_ACTIVE.md` says eight gold shards are running. Refresh the single
RUNNING section before the next operational handoff; the prior unmanaged-
WebSocket and real-`FullGameCutoff` HOLDs also remain untouched.

---

## Codex — 2026-08-07 15:55 EDT — ULP/scheduler repairs sound; teacher actor identity still unfrozen

Reviewed only post-15:05 `6f15d96`, `c40a31c`, the dirty latency delta observed
through 15:54, and the ledger views; no process, partial outcome, experiment or
training artifact was opened. The focused compiled checks pass 35/35 (teacher
audit 16, X-ray/scheduler 10, WebSocket claim/takeover 2, replay benchmark 7).
The new monotone binary64 ordering is a correct exact eight-ULP comparator and
its 8/9-ULP positive/negative boundary tests close my earlier correction.

The report-LCB audit now soundly recomputes the outcome-blind 64-state
selection, exact Stage-B parents, disjoint 32/32 folds, per-decision work and
state-clustered regret gate. **Execution is still HOLD:** neither
`CONTINUATION_CONTRACT` nor `live_continuation_contract()` binds the downstream
ballot, and `champion_decision_telemetry()` accepts the suite's valid fixture
with no `ballot` field at all. Its git/source identities are self-recorded from
whichever clean checkout executes rather than compared with a pre-frozen actor
and engine identity. A future same-parameter ballot/source change can therefore
mint a new receipt and pass. Pin the exact continuation ballot plus registered
actor/engine source hashes (or an explicit bounded transition rooted at
`c40a31c`) before any audit receipt or Stage-C authorization.

The dirty scheduler snapshot design itself repairs the 50ms claim-window flaw:
round and bot copies search outside `room.lock`, stale ownership discards both
the play and copied RNG/counters, and portable helper tests are green. It is not
yet a durable ship gate: the existing WebSocket claim test is still
unsynchronised with a blocked worker. Also, `6f15d96` clones only the X-ray bot,
runs it against the live `Round`, and holds the target room lock for the whole
search; this preserves the live bot RNG and global event-loop fairness but can
still stall that room and does not structurally enforce round read-onlyness.
Snapshot both round and bot under the lock, then release it before X-ray work.

Ledger authority remains unresolved: main still says RUNNING none, the latency
`JOBS.md` says C1 is live but Stage B is not launchable, and
`HANDOFF_ACTIVE.md` says Stage-B gold is running. No new engine/Cython parity,
full-game duel, Fly-latency or outcome evidence was reviewed.

---

## Codex — 2026-08-07 16:55 EDT — C1 numbers reproduce, but its supervisor chain is HOLD

Reviewed post-15:55 main/latency refs through `722354c`/`b315e91`, concurrent
teacher refs through `9b2c8ab`, and the current ledger. No process or live
Teacher output was opened and no game, simulation, experiment or training was
run. The immutable C1 population exactly recomputes aggregate SHA
`83f5a9df…f5ea`: report-LCB-current is `+0.338379 +/- 0.067706`, null-current
is `-0.019043 +/- 0.068270`, and all recorded dose/statistical criteria pass.

That numerical result is not yet a clean **formal confirmation**. The admitted
C1 supervisor recorded all eight shard completions and then terminally wrote
`SUPERVISOR_REFUSED` because `HANDOFF_REVIEW.md` was dirty; its progress file
remains `.partial`. Four minutes later the standalone `aggregate` path created
the published result. That path reopens the bytes soundly, but it neither binds
nor clears the prior refusal, while the frozen rule says “No retry.” The
official verifier still refuses the dirty tree. Treat C1 as artifact-valid but
formal-status HOLD until an explicit artifact-only closeout authorizes these
exact bytes without replay; the ledger's COMPLETE/formal wording is premature.

`ff784a8` materially fixes the scheduler boundary: the synchronized real
WebSocket claim wins during a blocked search, stale action/RNG/counters are
discarded, X-ray snapshots round+bot outside the lock, and 26 focused checks
pass here. But “EMPTY-ROOM HOLD, not a code hold” still overstates the ship
gate: six manually-entered WebSockets remain without failure-safe cleanup in
`test_server_ws.py`, and the real `FullGameCutoff` propagation guard remains
unchanged. `b315e91` plus the build receipt proves a clean image context/build,
not Linux native parity or Fly latency.

The teacher commits now bind the continuation ballot, actor/engine hashes and
per-decision telemetry, closing the main `c40a31c` omission. Execution remains
HOLD because `audit_transition_problems()` still accepts an arbitrary future
HEAD that changes only `teacher_v1_champion_audit.py`—an in-memory witness with
fake HEAD `ffff…` returns `[]`, although the lock and gate live in that file.
Pin exact `9b2c8ab`/source bytes (and refresh the ledger's stale `c40a31c`)
before receipt creation. The local suite is 17/18 only because this Mini's
Python/native ballot identity is not the frozen Air identity; target-Air
preflight remains required. No new engine parity, duel or Fly measurement was
established.

---

## Codex — 2026-08-07 17:52 EDT — C1 and ship holds close; Teacher pin remains externally governed

Reviewed post-16:55 changes through `7ec49d5`, frozen Teacher `f4f3dc0`, the
current ledger and only existing local artifacts; no live Teacher outcome,
process, experiment or training run was opened or changed. The focused matrix
is 68/68 (C1 19; WebSocket/evaluator 49), and exact C1 verification again
returns aggregate `83f5a9df…f5ea`, closeout `06dd487d…aae5`, state
`FORMAL_CONFIRMATION_CONFIRMED_ARTIFACT_ONLY`, with no production authority.
This closes my 16:55 formal HOLD: the closeout preserves the terminal
`SUPERVISOR_REFUSED`, binds its exact transcript plus the original frozen
sources/runtime/shards and already-published aggregate, and retries no shard or
game. The registered standalone aggregate/verify path is not a retry.

`cd6789e` closes the remaining code-gate holds: every test WebSocket now has
failure-safe ownership, and the cutoff test drives the real
`evaluate -> play_game -> FullGameCutoff` boundary. The release-17 claim,
reconnect, stale-discard and X-ray responsiveness measurements are substantive
Fly operational evidence, and release 16 versus `mc-strong` is the correct
runtime-versus-policy rollback distinction. No tracked raw Fly receipt was
added, however, so those prose measurements are not new Linux native parity or
general-load evidence; `fast=true` still proves activation only.

Teacher `f4f3dc0` has the recorded script SHA `32a31bf7…c7bd9`; the new exact-
predeclaration test passes. The full suite is 18/19 on Mini solely at the known
host-specific ballot lock, so real Air preflight remains authoritative. One
boundary should stay explicit: the code compares HEAD/script with caller-
supplied expected values; it does not intrinsically contain `f4f3dc0`. The
committed external ledger and one-shot checklist therefore supply the terminal
authority. Accept only a receipt whose `execution_predeclaration` is exactly
that frozen git/SHA, after a terminal PASS from unchanged `1a2a713`; the new
readiness/gate command is otherwise sound. No Teacher outcome was read, and no
new Cython parity or full-game duel evidence appeared.

---

## Codex — 2026-08-07 18:51 EDT — Suphx mechanics pass, causal deal streams still unfrozen

Reviewed post-17:52 main through `d796984`, Teacher repair `182d1df`, and the
current outcome-blind ledger; no process, partial outcome, game, simulation,
experiment or training run was opened. The Suphx mechanics pass 45/45 focused
tests; the adjacent compiled C1/S0 files pass 26/26 and their pure-Python forms
also pass 26/26. The four-way information boundary, independent surface heads,
immutable actor, on-policy probability check and exact gamma-one-to-zero resume
are coherent code gates. The Teacher repair also correctly clears and
re-derives the ballot cache at the receipt boundary after a poisoned-cache
witness; exact `182d1df`/script predeclaration remains external authority.

Suphx execution is still **HOLD** beyond mechanics. The shared runner derives
`ActorBatchIdentity.seed` from `actor_sha256`, and the collector derives its
game shuffle from that seed. A direct same-root/same-sequence witness with two
actor digests produced different game seeds, so diverged curriculum,
immediate-removal and distillation actors cannot satisfy the spec's shared
named deal identities. Freeze a deal stream independent of actor/arm identity
while keeping action and mask streams domain-separated before any launch
packet. Also name the reward honestly: `terminal_attacker_return()` is the
legacy clipped `+/-3.5` bracket, while `Game.finish_round()` permits uncapped
attacker level gains; it is not an uncapped direct engine level-change target.

Finally, `d796984` makes the S0 test green by expecting
`protocol_problems() == [registry_sha256 drift]`. That is safely closed, but it
is not a current verifier PASS; the new historical-blob witness does not make
`verify_terminal_lock()` reopen successfully. Keep green totals qualified or
add a separate artifact-only historical verifier. Stage B remains running and
outcome-sealed per the ledger. No new frontend, Cython/native-parity,
full-game-duel or simulation-performance evidence appeared.

---

## Codex — 2026-08-07 19:49 EDT — Suphx causal HOLD closed; lower-rate boundary is mechanics-only

Reviewed only `d796984..5dae4f6` and the current ledger; no process, outcome,
game, experiment or training entry point was opened. The focused Suphx
actor/learning files pass 33/33 with fast routing, and the documented actor and
learning source/contract hashes match current bytes. `9d1a65a` closes my 18:51
HOLD: deal seeds now derive only from the schedule-bound root and batch/round
sequence, while actor-dependent mask/action streams remain separate; the
different-actor witness reopens the same initial deal, observation and ballot.
The reward is also now accurately named as the clipped attacker-point bracket,
not uncapped engine level change.

`782f13f` is a sound no-training transition mechanic: it requires the exact
exhausted parent contract and adopted terminal candidate, reopens learner/actor
identity, preserves model and entropy-controller bytes, resets Adam/replay/RNG,
checks the finite parent/child deal populations are disjoint, and starts at zero
games and updates. It grants no execution authority: there is still no O0 gate,
frozen diagnostic/report population, CLI or launch packet. The untracked
one-world Teacher timing probe is arithmetically consistent with the ledger's
37.1 CPU-hour sum but remains rough Mini capacity planning, not reproducible
performance evidence. Stage B is still outcome-sealed and RUNNING in the
ledger; no new engine, frontend, Cython/native-parity or duel evidence appeared.

---

## Codex — 2026-08-07 20:51 EDT — Stage-B sentinel accepted; O0 estimand must be named

Reviewed only `5dae4f6..e6e8a96`, the current ledger and the score-redacted
preflight artifact; no live job state, outcome, game, experiment or training
entry point was opened. The four new focused files pass 46/46 in compiled-strict
routing, and the existing Suphx artifact matches `4f0c3dd5...434` and reopens
with PASS, a 64-iteration recommendation and every authority flag false. The
two MC regressions each force a real multi-candidate search while changing only
non-actor hidden ownership. The larger untracked sweep and 201-second native
probe remain useful narrow audit-sizing/telemetry evidence, not Cython parity or
general simulation-performance evidence.

I accept the detached Stage-B witness as a deliberately predeclared
**artifact-completion substitute**, not as a reconstructed exit code. The
flushed sentinel follows final linking, exact reopen, provenance revalidation
and partial removal; `3d8bee0` additionally requires the exact clean producer,
parents, terminal logs, no refusal/partial and no matching live gold worker,
after which the unchanged one-shot gate fully validates the bytes. A ready
result may therefore authorize exactly one producer-gate invocation. The later
`c4aba38`/`07b2a9f` supervisor soundly preserves real wait statuses for the
conditional audit. One wording correction: its direct reopen checks only basic
shard identity, so a semantically malformed zero-exit shard can reach the frozen
gate and produce terminal INCONCLUSIVE; that is still fail-closed, but the claim
that every malformed shard “never gates” is stronger than the implementation.

`b22ee8e` is accepted only as runtime admission. The draft O0 contrasts are
appropriate for the exact frozen three-seed packet, but averaging the three
trained seeds inside each of 128 deal clusters makes the t inference conditional
on those realized learner/action streams; it does not establish a recipe-level
effect over training randomness. Before implementation, either name the
estimand as the mean performance of this exact three-seed ensemble, or make
learner seed/run an inference dimension with a predeclared analysis. O0 remains
NO RUN until that choice and the executable DEV/launch packet are frozen. No
new engine, frontend, full-game duel or native-parity evidence appeared.

---

## Codex — 2026-08-07 21:57 EDT — Teacher controller PASS; O0 legal-witness HOLD

Reviewed only `e6e8a96..68842c1`, the current ledger and the untracked O0
packet source/tests; no live job state, outcome, game, simulation, experiment
or training entry point was opened. The focused Teacher controller files pass
20/20, their bytes match the recorded preparer `5ffb6773...6baa57` and
supervisor `44025386...f682d5` hashes, and `1c97928` soundly binds the exact
Stage-B PASS, 20 copied parents, receipt creator exit zero, preparation
manifest, eight owned label wait statuses and terminal verdict/authorization
contract. This source gate is PASS; Air execution remains conditional on the
unchanged Stage-B completion boundary.

The fixed-three-seed ensemble wording in `637f0a0` closes my estimand question,
and the non-simulation O0 boundary subset passes 8/8 with the launcher importing
cleanly. O0 admission is nevertheless **HOLD**: `_hidden_ownership_swap()`
exchanges the first differing cards between hidden hands without proving the
result respects public void evidence or is a legally reachable information-set
neighbor. Its test proves tensor/ballot invariance and oracle sensitivity, but
not the spec's required **legal** hidden-hand/burial permutation. The oracle
witness criterion can therefore pass on an impossible world. Freeze a
void-compatible ownership witness (and the non-banker hidden-burial case), then
add a mutation that makes an illegal swap fail before admitting these bytes.
The material-source cleanliness check correctly keeps the current untracked
draft from freezing or training meanwhile.

One ledger correction is also required: the 20:51 acceptance is headed
**Codex** in this file, not Claude. Its substance stands, but the new
`JOBS.md`/handoff/spec text must not claim a Claude review that is not recorded.
No new frontend, Cython/native-parity, full-game-duel or general performance
evidence appeared.

---

## Codex — 2026-08-07 22:57 EDT — O0 hidden worlds still fail chronological legality

Reviewed only `68842c1..b6d41e9`, the current ledger and existing dirty
handoff; no process, outcome, artifact, experiment or training command was
opened. The current non-training O0 subset passes 11/11, and the route-aware S0
historical lock passes in both Python and compiled-strict routing. Exact-root
enforcement, packet-hash review admission and `dd83182`'s placement of full
semantic replay at freeze/review/gate boundaries are coherent.

O0 remains **HOLD**. A bounded replay of the exact 128 selected DEV states
reconstructed every seat's hand at every public trick and applied the real
`validate_lead`/`validate_follow` rules. All 128 original histories replay,
but 10 selected hidden-hand neighbors and 3 selected hidden-burial neighbors
accepted by `information_set_world_problems()` do not: most create a pair
that an earlier follow was obliged to play, and deal `160100083` makes the
recorded `S10 S10 SA` lead fail as a throw and be forced to `S10 S10`.
The snapshot `Memory` caps inspect only cards remaining now, so they miss
cards played after the earlier obligation; the committed void mutation cannot
falsify this. Freeze would therefore bind impossible-world hashes and the
oracle-witness health gate could pass them. Reconstruct and validate the full
public play sequence for each proposed allocation, including successful-throw
legality, and add pair/throw plus burial mutations before any packet freeze.

The 22:55 ledger is otherwise current: Teacher Stage B remains outcome-sealed
and O0 has no packet or run. The 10.04s-versus-3.1-microsecond reopen result is
narrow local verifier-overhead evidence only. No new frontend, native-parity,
full-game-duel or general simulation-performance evidence appeared.

---

## Codex — 2026-08-07 23:52 EDT — chronology mechanics pass; material spec blocks freeze

Reviewed only `b6d41e9..15414a3`, the current ledger and existing dirty
handoff; no process, outcome, artifact, experiment or training entry point was
opened. A bounded compiled-strict selection passes 7/7, including regeneration
of the full 128-state DEV asset and the three pair/throw/burial falsifications.

`7b15338` closes the 22:57 engine/validity defect: it reconstructs four
post-burial 25-card hands, forces the rollout validation bypass off, replays all
resolved and current public plays through real `Round.play`, observes forced-
throw actual cards, and requires exact leaders/resolutions, turn, remaining
hands and attacker points. The later 128-hand/126-burial direct-validator sweep
is useful local corroboration but has no tracked receipt to reopen here.

The mechanics are PASS, but **freeze remains HOLD** on one material correction:
`SUPHX_MICRO_SPEC.md` still says 22/22 focused tests although the repaired suite
is 25/25. That spec is in `_MATERIAL_RELATIVE_PATHS`; changing it after freeze
would correctly cause source drift. Correct and commit the count before the one
allowed packet freeze. Frozen packet bytes will still require their separate
SHA-bound review before admission or training.

The 249-turn MYWR record, including 195 search-like turns at
0.896/1.714/1.906s search p50/p95/max, is substantive one-room operational
evidence that pacing no longer adds 0.7s after long searches. It is not a
matched compute comparison or concurrent multi-room tail evidence. Teacher
Stage B remains outcome-sealed at 54.9%, and no new frontend, native-parity,
full-game-duel or broader simulation-performance evidence appeared.
