# Fleet job ledger snapshot through 2026-08-08 08:43 EDT

This is the exact pre-compaction body of top-level `JOBS.md`. Original
SHA-256: `26beff936f6c0744b220fc79e233163c8f09acde8a13adcba5450327ad132252`. Current live status and terminal stubs remain in
`../JOBS.md`; dated synthesis lives in the daily logs.
...
`1c97928`. Preparer SHA `5ffb6773...6baa57` admits only an exact terminal PASS,
copies and rehashes all 20 parents, runs receipt creation synchronously and
persists exit zero. Supervisor SHA `44025386...f682d5` requires that preparation
manifest before owning all eight 32/32 labels and one terminal gate. Chained
focused tests pass 20/20; the adjacent Teacher matrix passes 154/154 in ordinary
and compiled-strict routing, and the broad ordinary suite passes 1,005 with 27
skips. The preparer verifies the actual supervisor bytes before copying; the
supervisor accepts only exact terminal verdict/authorization combinations. A
live Air preflight bound both evidence checkouts, both controller hashes, audit
...
Mini also ran a separate outcome-redacted capacity probe over the 64 already-
frozen audit states. One fresh `timing_probe_v1` world per state, candidate
zero only, measured no Stage-B label or choice and created no artifact. Scaling
those timings by each live ballot size and the literal 64 outer worlds projects
37.1 CPU-hours total and per-shard Mini times of
`2.27/6.09/2.03/4.01/5.49/6.78/4.61/5.87` hours. Treat the 6.8-hour critical
