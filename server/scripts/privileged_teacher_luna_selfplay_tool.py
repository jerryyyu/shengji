#!/usr/bin/env python3
"""Strict command-line bridge for one PT-Luna team mailbox."""
...
if not sys.flags.safe_path or not sys.dont_write_bytecode:
    raise RuntimeError("PT-Luna tool requires Python -P -B")

from shengji.rl.privileged_teacher_luna_selfplay_execution import tool_request
...
def main() -> int:
    ...
    play.add_argument("--confidence", choices=("low", "medium", "high"), required=True)
    args = parser.parse_args()
    if args.operation in ("observe", "wait"):
        request = {"op": args.operation}
    elif args.operation == "rollout":
        request = {"op": "rollout", "decision_sha256": args.decision,
                   "candidate_indices": args.candidates,
                   "continuations": args.continuations}
    else:
        request = {"op": "play", "decision_sha256": args.decision,
