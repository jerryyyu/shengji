#!/usr/bin/env python3
"""Run the bounded adaptive PT-Cla0 diagnostic on Mini's open-DEV roots.

Mirror of ``run_privileged_teacher_sol0.py`` with the planner swapped to the
Claude CLI.  All hardened I/O helpers, the strict-report parser, the
exclusive publisher and the frozen-design gate are imported from the
reviewed Sol0 runner rather than forked, so the two runners cannot drift
apart on those surfaces.
"""
...
if not sys.flags.safe_path or not sys.dont_write_bytecode:
    raise RuntimeError("PT-Cla0 runner requires Python -P -B")
...
def main() -> int:
    ...
    if os.environ.get("PYTHONPATH"):
        raise ValueError("PT-Cla0 runner refuses PYTHONPATH")
    if (os.environ.get("SHENGJI_FAST") != "1"
            or os.environ.get("SHENGJI_REQUIRE_VOIDS") != "1"):
        raise ValueError("PT-Cla0 runner requires strict native mode")
    ...
    if (_git(repo, "rev-parse", "HEAD") != args.expected_git
        ...
        raise ValueError("PT-Cla0 source or Mini host identity drift")
    for ancestor in (args.expected_c0_git, args.expected_full_git):
        if subprocess.run(
            ...
            raise ValueError("PT-Cla0 source ancestry drift")
    ...
    if (c0_external != args.expected_c0_external_sha256
        ...
        raise ValueError("PT-Cla0 parent external identity drift")
    ...
    if (c0_report.get("report_sha256") !=
        ...
        raise ValueError("PT-Cla0 parent internal identity drift")
    ...
    design = Cla0Design(
        ...
        codex_binary_sha256=hashlib.sha256(
            claude_path.read_bytes()).hexdigest(),
    ...
    def progress(row: dict[str, object]) -> None:
        print("PT_CLA0_PROGRESS " + json.dumps(
    ...
    def execute_bound_design():
        ...
        return run_dev(
            ...
            tool_script=tool_script, codex_binary=claude_path,
