"""Blind, champion-matched screen and confirmation for the V11 anchor.

The screen requires the exact frozen ``v11_revalidate.py`` aggregate as a
protocol and matched-null sanity parent.  It preserves that aggregate's
standalone authorization verdict verbatim but does not require it to be true:
a direct policy can be weaker than a search policy while still providing a
useful protected proposal.  The policy reference comes only from the
independently verified terminal S0 packet.
...
DIRECT_GIT_SHA = "e66b90bc3a50d514472670ea99909add5ea30d19"
DIRECT_SOURCE_SHA256 = {
    "runner": "37d67e164aa84042b6855982ef0c113a3af7338b569122ddc0c2818cbdb51ce5",
    "evaluation": "4b75a5e643c8a2514c6d14707085f89a33e0c534adc2989229894f2f219a6b16",
    "registry": "7e2074ee0b1098ecf79f57379c82aae325325b797796b5144db7825437586a8d",
    ...
    "torch_policy": "e19c99060d8ae095a7a420ab8b14b36cdb9defeb3ee2e98319bae89fe831e595",
    "encoder": "a2022b0754597627ba58c2c4630754ae89b5be0b4daefe7a775b6ade587214cf",
...
def direct_aggregate_problems(payload: dict) -> list[str]:
    ...
    if (payload.get("clusters") != DIRECT.TOTAL_CLUSTERS
        ...
        problems.append("direct aggregate exact 121M coverage")
    ...
    if (not isinstance(shards, list) or len(shards) != DIRECT.SHARD_COUNT
            or len(set(shards)) != DIRECT.SHARD_COUNT):
    ...
    try:
        derived = DIRECT.gate_criteria(stats)
    ...
    if not isinstance(payload.get("anchor_test_authorized"), bool) \
            or (derived is not None and
                payload.get("anchor_test_authorized") is not derived["all"]):
        problems.append("direct aggregate original authorization verdict")
    ...
    if payload.get("production_promotion") is not False:
        problems.append("direct aggregate claims production promotion")

    counters = payload.get("counter_totals", {})
    if set(counters) != set(DIRECT.LABELS):
        ...
        for label, sides in counters.items():
            ...
            for side, values in sides.items():
                if not isinstance(values, dict):
                    problems.append(f"direct aggregate {label}/{side} counters")
                    continue
                attempts = values.get("sample_attempts")
                accepted = values.get("accepted_worlds")
                failed = values.get("failed_worlds")
                if (not all(isinstance(value, int) and value >= 0
                            for value in (attempts, accepted, failed))
                        or attempts != accepted + failed):
                    problems.append(
                        f"direct aggregate {label}/{side} counter reconciliation")
                for field in ("failed_worlds", "rejected_worlds",
                              "short_searches", "zero_world", "void_fallbacks"):
                    if not isinstance(values.get(field), int) or values.get(field):
                        problems.append(
                            f"direct aggregate {label}/{side} nonzero/bad {field}")
    return sorted(set(problems))
...
def load_direct_parent(path: os.PathLike | str,
    ...
    return {
        ...
        "anchor_test_authorized": payload["anchor_test_authorized"],
...
def load_screen_parent(path: os.PathLike | str, expected_sha256: str,
    ...
    if (not isinstance(direct_parent, dict)
            ...
            or not isinstance(direct_parent.get("anchor_test_authorized"), bool)
            ...
            or direct_parent.get("anchor_test_authorized") is not
