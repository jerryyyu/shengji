...
def _preimport_bytecode_scan() -> None:
    """Scan bound source parents before importing any project module."""
    if "--bootstrap-check-only" in sys.argv or "--freeze" not in sys.argv:
        return
    index = sys.argv.index("--freeze")
    if index + 1 >= len(sys.argv):
        raise RuntimeError("freeze path missing")
    try:
        value = json.loads(Path(sys.argv[index + 1]).read_text(encoding="ascii"))
        bindings = value["source_bindings"]
    except (OSError, UnicodeError, KeyError, TypeError, ValueError) as exc:
        raise RuntimeError("freeze cannot be scanned before import") from exc
    prefixes = {(REPO / row["path"]).parent.resolve() for row in bindings}
    for prefix in prefixes:
        ...
        for current, dirs, files in os.walk(prefix, topdown=True, followlinks=False):
            ...
            if "__pycache__" in dirs or any(name.endswith(".pyc") for name in files):
                raise RuntimeError("source bytecode artifact before project import")


_preimport_bytecode_scan()

from shengji.rl.world_afterstate_v2_execution import (  # noqa: E402
    ExecutionFreezeV2, StageSupervisorV2, WorldAfterstateV2ExecutionError,
