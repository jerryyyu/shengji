...
from .production_pv_control import ProductionPVControl, SHA256 as PRODUCTION_PV_SHA256
...
CAP = 4000
CONTROL_NAMES = ("mc-lcb", "mc-smart4", "policy-world", "policy-value", "production-play", "production-pv-r29")
...
def make_control(name: str, seed: int, production=None):
    ...
    if name in ("production-play", "production-pv-r29"):
...
def _decision_telemetry(bot, side: str) -> dict[str, Any]:
    if side == "policy":
        ...
        return {
            "fallback_decisions": int(record.get('schema') == 'pv-search-fallback-v1'),
            "budget_fallbacks": int(record.get('schema') == 'pv-search-fallback-v1' and record.get('reason') == 'budget'),
            "error_fallbacks": int(record.get('schema') == 'pv-search-fallback-v1' and record.get('reason') == 'search-error'),
...
def _worker_init(checkpoint: str, checkpoint_sha256: str, worlds: int,
    ...
    if production is not None:
        expected_sha = PRODUCTION_PV_SHA256 if control == 'production-pv-r29' else PRODUCTION_SHA256
        if _sha256(Path(production.checkpoint)) != expected_sha:
...
def _validate_args(args) -> None:
    if (args.control in ('production-play', 'production-pv-r29')) != bool(args.production_checkpoint):
...
def main(argv=None) -> int:
    ...
    if args.production_checkpoint:
        ...
        expected_sha = PRODUCTION_PV_SHA256 if args.control == 'production-pv-r29' else PRODUCTION_SHA256
        if _sha256(production_path) != expected_sha:
            raise ValueError('production checkpoint SHA256 mismatch')
        cls = ProductionPVControl if args.control == 'production-pv-r29' else ProductionPlayControl
        production = cls(str(production_path))
