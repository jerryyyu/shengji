...
def test_zero_enabled_projection_still_requires_allowed_row_support():
    _, actor, _, _ = _state(9999)
    weights = uniform_raw_count_weights(
        actor, behavior_policy_ids=POLICIES)
    row = next(candidate for candidate in weights
               if any(candidate.count_weights))
    index = weights.index(row)
    unsupported = (*weights[:index], replace(
        row, count_weights=(0, 0, 0)), *weights[index + 1:])
    with pytest.raises(BeliefProjectionError,
                       match="no allowed count support"):
        project_count_weights(
            actor,
            behavior_policy_ids=POLICIES,
            model_schema="history-ownership-v1-test",
            model_sha256="d" * 64,
            raw_weights=unsupported,
