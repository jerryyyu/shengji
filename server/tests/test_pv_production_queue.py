...
SCRIPT = Path(__file__).parents[1] / "scripts" / "queue_pv_production_screen.sh"
INVOCATION = "4065f2b75f7d4cc298e82bf01fdff710"
...
def test_final_guard_precedes_production_exec():
    text = SCRIPT.read_text()
    assert 'read_snapshot\nrequire_success\nrequire_launcher\nexec "$python" "$launcher"' in text
    assert "--suite pv-production-screen --run" in text
...
def test_output_identity_and_completion_guard(tmp_path, damage):
    ...
    plan = b'{"suite":"world-scaling-screen"}'
    ...
    program = program.replace("Path('/root/codex-pv-world-scaling-800-20260920')", f'Path({str(tmp_path)!r})')
    inode = identity.st_ino + int(damage == 'directory')
    program = program.replace('(2049, 1051821)', repr((identity.st_dev, inode)))
    program = program.replace('521a9d5d56d0a5cb625b077467d15884c53c07a72a673c8756099392b9bf9345',
...
def test_full_queue_flow(tmp_path, scenario):
    ...
    if scenario == 'launcher_tamper':
        ...
        launcher.unlink()
    active = snapshot(ActiveState='active', SubState='running', MainPID='1416118')
...
'''
    ...
    script = script.replace('d83f8c25b5f8ef63b32e47b57bbc911bcbe1893f95c5add6462c5c119e093dd1', launcher_sha)
    script = script.replace('python=/root/gen-hybrid/server/.venv/bin/python',
                            f'python={shlex.quote(sys.executable)}')
    script = script.replace('launcher=/root/codex-production-full-launcher-20260920/server/scripts/policy_abc_launcher.py',
                            f'launcher={shlex.quote(str(launcher))}')
    script = script.replace("Path('/root/codex-pv-world-scaling-800-20260920')", f'Path({str(root)!r})')
    identity = root.stat()
    script = script.replace('(2049, 1051821)', repr((identity.st_dev, identity.st_ino)))
    script = script.replace('521a9d5d56d0a5cb625b077467d15884c53c07a72a673c8756099392b9bf9345',
                            hashlib.sha256(plan).hexdigest())
    if scenario == 'deadline':
        script = script.replace('SECONDS + 66000', 'SECONDS - 1')
    result = subprocess.run(['bash', '-c', script], capture_output=True, text=True, timeout=10)
    if scenario == 'success':
        assert result.returncode == 0, result.stderr
        args = json.loads(launched.read_text())
        assert args[-3:] == ['--suite', 'pv-production-screen', '--run']
